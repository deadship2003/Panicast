...
bool rename_if_needed(const std::string& oldp, const std::string& newp) {
    if (oldp.empty() || newp.empty()) return false;
    std::error_code ec;
    if (fs::exists(newp, ec)) return false;   // already migrated (or in use)
    if (!fs::exists(oldp, ec)) return false;   // nothing to migrate
...
void Paths::migrate_legacy() {
    ...
    if (rename_if_needed(h + "/.config/podradio", h + "/.config/panicast")) moved = true;
    ...
    if (moved) fix_config_paths(h + "/.config/panicast/config.ini");
}
