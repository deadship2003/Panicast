...
std::string Utils::which_binary(const std::string& name) {
    ...
    for (const char* fb : fallbacks) {
        struct stat st;
        if (stat(fb, &st) == 0 && S_ISREG(st.st_mode) && access(fb, X_OK) == 0 && std::string(fb).find(name) != std::string::npos) {
            return fb;
        }
    }
    return "";
...
void Utils::emit_osc8_link(int scr_row, int scr_col, const std::string& visible,
    ...
    if (url.empty() || scr_row < 1 || scr_col < 1) return;
...
bool Utils::copy_to_clipboard(const std::string& text) {
    struct Tool { const char* name; const char* a1; const char* a2; };
    static const Tool tools[] = {
        {"wl-copy",  nullptr,       nullptr},
        {"xclip",    "-selection",  "clipboard"},
        {"xsel",     "--clipboard", "--input"},
        {"pbcopy",   nullptr,       nullptr},
    ...
    for (const auto& t : tools) {
        ...
        while (std::getline(ss, dir, ':')) {
            ...
            if (is_exec(candidate)) { exe = candidate; a1 = t.a1; a2 = t.a2; break; }
        }
        if (exe.empty() && is_exec(t.name)) { exe = t.name; a1 = t.a1; a2 = t.a2; }
        if (!exe.empty()) break;
    }
    ...
    if (a1) storage.push_back(a1);
    if (a2) storage.push_back(a2);
    ...
    close(in_pipe[0]);
    if (rc != 0) { close(in_pipe[1]); return false; }
...
Utils::ProcResult Utils::run_process(const std::string& exe, const std::vector<std::string>& args) {
    ...
    if (pid < 0) { close(out_pipe[0]); close(err_pipe[0]); return res; }
    res.launched = true;
...
int Utils::run_process_streaming(const std::string& exe, const std::vector<std::string>& args,
    ...
    if (pid < 0) { close(out_pipe[0]); return -1; }
    Utils::register_child_pid(pid);  // N04-fix: track for shutdown kill
    ...
    if (fp) {
        ...
        while ((n = getline(&line, &cap, fp)) > 0) {
            if (stop_pred && stop_pred()) { stopped = true; break; }
            if (line_cb) {
    ...
    if (stopped) { kill(-pid, SIGTERM); }  // N04-fix: group kill
    int status = 0;
    waitpid(pid, &status, 0);
    Utils::unregister_child_pid(pid);
