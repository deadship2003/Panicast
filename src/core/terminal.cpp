...
bool TerminalDetector::narrow_ambiguous() {
    ...
    return t == TerminalType::WINDOWS_TERMINAL ||
           t == TerminalType::CMD_CONSOLE;
}

int TerminalDetector::recommended_emoji_width() {
    TerminalType t = detect();
    if (t == TerminalType::WINDOWS_TERMINAL ||
        t == TerminalType::CMD_CONSOLE) {
        return 1;
    }
    return 2;
...
std::string TerminalDetector::terminal_name() {
    TerminalType t = detect();
    switch (t) {
        case TerminalType::XTERM:             return "xterm/Linux";
        case TerminalType::WINDOWS_TERMINAL:  return "Windows Terminal";
        case TerminalType::CMD_CONSOLE:       return "Windows cmd.exe";
        case TerminalType::APPLE_TERMINAL:    return "Apple Terminal";
...
int get_emoji_width() {
    ...
    if (g_emoji_width_override > 0) return g_emoji_width_override;
    if (g_emoji_width_probed > 0) return g_emoji_width_probed;
...
int probe_emoji_width() {
    ...
    if (tcgetattr(stdin_fd, &old_tio) != 0) return 0;
    ...
    if (tcsetattr(stdin_fd, TCSANOW, &new_tio) != 0) return 0;
    ...
    if (write(STDOUT_FILENO, probe, probe_len) == (ssize_t)probe_len) {
        ...
        if (poll(&pfd, 1, 300) > 0) {
            while (idx < 31) {
                ...
                if (reply[idx - 1] == 'R') break;  // End of reply
                struct pollfd p2{ stdin_fd, POLLIN, 0 };
                if (poll(&p2, 1, 50) <= 0) break;  // Subsequent bytes did not arrive in time
            }
        }
        reply[idx] = '\0';
