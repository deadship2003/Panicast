...
void Persistence::save_cache(const std::vector<TreeNodePtr>& radio, const std::vector<TreeNodePtr>& podcast) {
...
void Persistence::load_cache(std::vector<TreeNodePtr>& radio, std::vector<TreeNodePtr>& podcast) {
    ...
    tmp->title = "Radio"; tmp->type = NodeType::FOLDER; tmp->expanded = true;
...
void Persistence::save_data(const std::vector<TreeNodePtr>& podcasts, const std::vector<TreeNodePtr>& favs) {
    ...
    for (const auto& p : podcasts) if (p && !p->url.empty()) pod_roots.push_back(p);
    ...
    for (const auto& f : favs) {
        if (f) {
            db.save_favourite(
                f->title, f->url, (int)f->type, f->is_youtube, f->channel_name,
...
void Persistence::load_data(std::vector<TreeNodePtr>& podcasts, std::vector<TreeNodePtr>& favs) {
    ...
    for (const auto& [title, url, type, is_youtube, channel_name, source_type, is_link, link_target_url, is_local_folder, mt] : db_favs) {
