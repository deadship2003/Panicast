...
YouTubeCache& YouTubeCache::instance() { static YouTubeCache yc; return yc; }
...
void YouTubeCache::update(const std::string& url, const std::string& name, const std::vector<YouTubeVideoInfo>& videos) {
    std::lock_guard<std::mutex> lock(mtx_);
...
bool YouTubeCache::load_from_db(const std::string& channel_url) {
    ...
    if (!DatabaseManager::instance().youtube_cache_load(channel_url, name, videos_json)) return false;
