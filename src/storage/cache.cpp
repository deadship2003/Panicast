...
CacheManager& CacheManager::instance() { static CacheManager cm; return cm; }
...
void CacheManager::mark_partial(const std::string& url) {
    if (url.empty()) return;
    std::lock_guard<std::mutex> lock(mtx_);
    if (downloaded_.count(url)) return;  // already complete — don't downgrade
    partial_.insert(url);
...
void CacheManager::clear_download(const std::string& url) {
    std::string path;
    {
        ...
        partial_.erase(url);
        auto it = local_files_.find(url);
        if (it != local_files_.end()) { path = it->second; local_files_.erase(it); }
...
void CacheManager::load() {
    std::vector<DatabaseManager::MediaCacheRow> rows;
    {
        ...
        for (const auto& r : rows) {
            if (r.status == 1) {
                ...
                if (!r.local_file.empty()) local_files_[r.url] = r.local_file;
            } else if (r.status == 2) {
    ...
    LOG(fmt::format("[CACHE] Loaded {} media-cache rows (complete={}, partial={})",
