...
void DatabaseManager::save_search_cache(const std::string& query, const std::string& region, const std::string& result_json) {
    if (!is_ready()) return;
    std::lock_guard<std::recursive_mutex> lock(mtx_);
    ...
    if (exists) {
        ...
        const char* update_sql = "UPDATE search_cache SET result_json=?, timestamp=CURRENT_TIMESTAMP "
...
std::string DatabaseManager::load_search_cache(const std::string& query, const std::string& region) {
    if (!is_ready()) return "";
    std::lock_guard<std::recursive_mutex> lock(mtx_);

    // Parameterized query (no string interpolation)
    const char* sql = "SELECT result_json FROM search_cache WHERE query=? AND region=? ORDER BY timestamp DESC LIMIT 1;";
...
void DatabaseManager::save_search_cache_src(const std::string& source, int account_id,
                                            const std::string& query, const std::string& result_json) {
    if (!is_ready()) return;
    std::lock_guard<std::recursive_mutex> lock(mtx_);
    int max_cache = IniConfig::instance().get_search_cache_max();
    sqlite3_stmt* stmt = nullptr;
    ...
    const char* chk = "SELECT id FROM search_cache WHERE source=? AND account_id=? AND query=? LIMIT 1;";
    bool exists = false;
    ...
    const char* sql = exists
        ? "UPDATE search_cache SET result_json=?, timestamp=CURRENT_TIMESTAMP WHERE source=? AND account_id=? AND query=?;"
        : "INSERT INTO search_cache (source, account_id, query, result_json) VALUES (?, ?, ?, ?);";
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
        if (exists) {
            sqlite3_bind_text(stmt, 1, result_json.c_str(), -1, SQLITE_TRANSIENT);
    ...
    {
        const char* cleanup = "DELETE FROM search_cache WHERE source=? AND account_id=? AND id NOT IN "
                              "(SELECT id FROM search_cache WHERE source=? AND account_id=? ORDER BY timestamp DESC LIMIT ?);";
        sqlite3_stmt* stmt = nullptr;
        if (sqlite3_prepare_v2(db_, cleanup, -1, &stmt, nullptr) == SQLITE_OK) {
            sqlite3_bind_text(stmt, 1, source.c_str(), -1, SQLITE_TRANSIENT);
            sqlite3_bind_int(stmt, 2, account_id);
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            if (j) {
                try {
                    ...
                    if (jj.contains("results") && jj["results"].is_array()) it.result_count = (int)jj["results"].size();
                } catch (...) {}
            }
            out.push_back(it);
        }
...
std::string DatabaseManager::load_search_cache_src(const std::string& source, int account_id,
    ...
    sqlite3_stmt* stmt = nullptr;
    std::string out;
    const char* sql = "SELECT result_json FROM search_cache WHERE source=? AND account_id=? AND query=? LIMIT 1;";
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, source.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_int(stmt, 2, account_id);
        sqlite3_bind_text(stmt, 3, query.c_str(), -1, SQLITE_TRANSIENT);
...
void DatabaseManager::save_episode_cache(const std::string& feed_url, const std::string& episodes_json) {
    if (!is_ready()) return;
    std::lock_guard<std::recursive_mutex> lock(mtx_);
    ...
    try {
        ...
        if (j.is_array()) {
            const char* sql = "INSERT INTO episode_cache "
                "(feed_url, episode_url, title, duration, pub_date, is_youtube, has_subtitle, subtitle_url, has_asr_srt, asr_srt_path) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
            sqlite3_stmt* stmt = nullptr;
            if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
                for (const auto& ep : j) {
                    ...
                    bool ep_is_youtube = ep.value("is_youtube", false);  // F38: column (was in data_json)
        ...
        if (own_txn) rollback_txn();  // Rollback on parse failure, otherwise DELETE commits while INSERT is empty -> cache cleared
        return;
    }
...
void DatabaseManager::update_episode_subtitle(const std::string& feed_url, const std::string& episode_url,
                                               bool has_subtitle, const std::string& subtitle_url) {
    if (!is_ready() || feed_url.empty() || episode_url.empty()) return;
    std::lock_guard<std::recursive_mutex> lock(mtx_);
    sqlite3_stmt* stmt = nullptr;
    if (sqlite3_prepare_v2(db_,
            "UPDATE episode_cache SET has_subtitle=?, subtitle_url=? WHERE feed_url=? AND episode_url=?;",
...
void DatabaseManager::update_episode_asr(const std::string& feed_url, const std::string& episode_url,
                                          const std::string& asr_srt_path) {
    if (!is_ready() || feed_url.empty() || episode_url.empty()) return;
    std::lock_guard<std::recursive_mutex> lock(mtx_);
    sqlite3_stmt* stmt = nullptr;
    if (sqlite3_prepare_v2(db_,
            "UPDATE episode_cache SET has_asr_srt=1, asr_srt_path=? WHERE feed_url=? AND episode_url=?;",
            -1, &stmt, nullptr) == SQLITE_OK) {
...
std::vector<std::tuple<std::string, std::string, int, bool, bool, std::string, bool, std::string>> DatabaseManager::load_episodes_from_cache(const std::string& feed_url) {
    std::vector<std::tuple<std::string, std::string, int, bool, bool, std::string, bool, std::string>> episodes;
    if (!is_ready()) return episodes;
    std::lock_guard<std::recursive_mutex> lock(mtx_);

    // Parameterized (no string interpolation)
    const char* sql = "SELECT episode_url, title, duration, is_youtube, has_subtitle, subtitle_url, has_asr_srt, asr_srt_path FROM episode_cache WHERE feed_url=? ORDER BY timestamp DESC;";
    ...
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            const char* sub_url = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 5));  // Y23.10
            bool ep_has_asr = sqlite3_column_int(stmt, 6) != 0;  // Y24.25: ASR SRT (📝)
            const char* asr_path = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 7));  // Y24.25
