...
bool DatabaseManager::youtube_cache_load(const std::string& channel_url, std::string& out_name,
    ...
    if (!is_ready() || channel_url.empty()) return false;
...
void DatabaseManager::youtube_cache_save(const std::string& channel_url, const std::string& channel_name,
                                         const std::string& videos_json, int account_id) {
    if (!is_ready() || channel_url.empty()) return;
    std::lock_guard<std::recursive_mutex> lock(mtx_);
    sqlite3_stmt* stmt = nullptr;
    const char* sql = "INSERT OR REPLACE INTO youtube_cache (account_id, channel_url, channel_name, videos_json, updated_at) "
    ...
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        sqlite3_bind_text(stmt, 4, videos_json.c_str(), static_cast<int>(videos_json.size()), SQLITE_TRANSIENT);
...
std::vector<BilibiliAccount> DatabaseManager::list_bilibili_accounts() {
    ...
    std::lock_guard<std::recursive_mutex> lock(mtx_);
    sqlite3_stmt* stmt = nullptr;
    if (sqlite3_prepare_v2(db_, "SELECT id, uid, uname, sessdata, bili_jct, dedeuserid FROM bilibili_accounts ORDER BY id", -1, &stmt, nullptr) == SQLITE_OK) {
...
int DatabaseManager::upsert_bilibili_account(const BilibiliAccount& a) {
    ...
    int existing_id = 0;
    {
        ...
        if (sqlite3_prepare_v2(db_, "SELECT id FROM bilibili_accounts WHERE uid=?;", -1, &chk, nullptr) == SQLITE_OK) {
            sqlite3_bind_text(chk, 1, a.uid.c_str(), -1, SQLITE_TRANSIENT);
            if (sqlite3_step(chk) == SQLITE_ROW) existing_id = sqlite3_column_int(chk, 0);
            sqlite3_finalize(chk);
        }
    }
    if (existing_id) {
        sqlite3_stmt* upd = nullptr;
        if (sqlite3_prepare_v2(db_, "UPDATE bilibili_accounts SET sessdata=?, bili_jct=?, dedeuserid=?, uname=?, last_login_at=datetime('now') WHERE id=?;", -1, &upd, nullptr) == SQLITE_OK) {
    ...
    if (sqlite3_prepare_v2(db_, "INSERT INTO bilibili_accounts (uid, uname, sessdata, bili_jct, dedeuserid, last_login_at) VALUES (?, ?, ?, ?, ?, datetime('now'));", -1, &ins, nullptr) == SQLITE_OK) {
...
bool DatabaseManager::delete_bilibili_account(int id) {
    if (!is_ready() || id <= 0) return false;
    std::lock_guard<std::recursive_mutex> lock(mtx_);
    sqlite3_stmt* del = nullptr;
    bool ok = false;
    if (sqlite3_prepare_v2(db_, "DELETE FROM bilibili_accounts WHERE id=?;", -1, &del, nullptr) == SQLITE_OK) {
...
std::vector<TiktokAccount> DatabaseManager::list_tiktok_accounts() {
    ...
    std::lock_guard<std::recursive_mutex> lock(mtx_);
    sqlite3_stmt* stmt = nullptr;
    if (sqlite3_prepare_v2(db_, "SELECT id, platform, handle, url, uname FROM tiktok_accounts ORDER BY id", -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            TiktokAccount a;
            a.id = sqlite3_column_int(stmt, 0);
...
int DatabaseManager::upsert_tiktok_account(const TiktokAccount& a) {
    ...
    {
        sqlite3_stmt* chk = nullptr;
        if (sqlite3_prepare_v2(db_, "SELECT id FROM tiktok_accounts WHERE platform=? AND handle=?;", -1, &chk, nullptr) == SQLITE_OK) {
    ...
    if (existing_id) {
        sqlite3_stmt* upd = nullptr;
        if (sqlite3_prepare_v2(db_, "UPDATE tiktok_accounts SET url=?, uname=? WHERE id=?;", -1, &upd, nullptr) == SQLITE_OK) {
    ...
    sqlite3_stmt* ins = nullptr;
    if (sqlite3_prepare_v2(db_, "INSERT INTO tiktok_accounts (platform, handle, url, uname) VALUES (?, ?, ?, ?);", -1, &ins, nullptr) == SQLITE_OK) {
...
bool DatabaseManager::delete_tiktok_account(int id) {
    ...
    sqlite3_stmt* del = nullptr;
    bool ok = false;
    if (sqlite3_prepare_v2(db_, "DELETE FROM tiktok_accounts WHERE id=?;", -1, &del, nullptr) == SQLITE_OK) {
        sqlite3_bind_int(del, 1, id);
        ok = sqlite3_step(del) == SQLITE_DONE;
        sqlite3_finalize(del);
    }
...
void DatabaseManager::save_bili_up(const std::string& mid, const std::string& uname,
    ...
    if (!is_ready() || mid.empty()) return;
    std::lock_guard<std::recursive_mutex> lock(mtx_);
    sqlite3_stmt* stmt = nullptr;
    const char* sql = "INSERT OR REPLACE INTO bilibili_up_cache (mid, uname, upic, fans, sign, updated_at) "
                      "VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP);";
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
...
std::string DatabaseManager::load_bili_up_pic(const std::string& mid) {
    ...
    sqlite3_stmt* stmt = nullptr;
    std::string pic;
    if (sqlite3_prepare_v2(db_, "SELECT upic FROM bilibili_up_cache WHERE mid=?;", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, mid.c_str(), -1, SQLITE_TRANSIENT);
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            const unsigned char* t = sqlite3_column_text(stmt, 0);
            if (t) pic = reinterpret_cast<const char*>(t);
        }
        sqlite3_finalize(stmt);
    }
    return pic;
