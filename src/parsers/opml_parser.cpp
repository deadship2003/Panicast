...
TreeNodePtr OPMLParser::parse(const std::string& xml) {
    ...
    XmlDocGuard doc(xmlReadMemory(xml.c_str(), xml.size(), "opml", NULL, XML_PARSE_NONET | XML_PARSE_NOERROR | XML_PARSE_NOWARNING));
    if (!doc) {
        // Log XML error
...
void OPMLParser::extract_feeds(const std::string& xml, std::vector<TreeNodePtr>& feeds) {
    ...
    if (!root) { return; }  // doc is auto-released by XmlDocGuard
