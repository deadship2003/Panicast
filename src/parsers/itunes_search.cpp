...
TreeNodePtr ITunesSearch::parse_result(const json& item) {
    ...
    if (name.empty()) name = get_str("trackName");
    if (name.empty()) return nullptr;
    node->title = name;

    node->url = get_str("feedUrl");
    if (node->url.empty()) return nullptr;  // Skip items without a feed URL

    // Add subtext to display more information
    std::string subtext;
    std::string artist = get_str("artistName");
    if (!artist.empty()) subtext = artist;
...
void ITunesSearch::save_podcast_to_cache(const json& item) {
    if (!item.contains("feedUrl") || !item["feedUrl"].is_string()) return;  // null/non-string feedUrl does not throw, to avoid discarding the whole batch
...
std::string ITunesSearch::fetch(const std::string& url) {
    ...
    curl_easy_setopt(curl, CURLOPT_SSL_ENABLE_ALPN, 1L);     // Enable ALPN - iTunes (Akamai CDN) requires it
