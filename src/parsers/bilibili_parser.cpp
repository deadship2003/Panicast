...
TreeNodePtr BilibiliParser::parse_user_videos(const std::string& sessdata, const std::string& mid,
    ...
    feed->children_loaded = true;  // a feed with no videos is still "loaded" (avoids infinite re-expand)
    if (mid.empty()) return feed;
...
std::vector<TreeNodePtr> BilibiliParser::parse_followings(const std::string& sessdata, const std::string& uid,
                                                           int account_id) {
    ...
    for (const auto& f : followings) {
        ...
        if (!f.sign.empty()) c->subtext = f.sign;
        out.push_back(c);
    }
    return out;
}

std::vector<TreeNodePtr> BilibiliParser::parse_history(const std::string& sessdata, int account_id) {
    std::vector<TreeNodePtr> out;
