...
namespace panicast
{
    ...
    void App::download_node(int marked_count) {
        std::vector<TreeNodePtr> items;
        {
            ...
            if (marked_count > 0) collect_playable_marked_current( items);
        ...
        for (auto& n : items) pending_downloads_.push_back(n);
...
void App::ytdlp_download(const std::string& url, const std::vector<std::string>& site_args,
        ...
        args.push_back("-f"); args.push_back("bestvideo+bestaudio/best");
        args.push_back("--merge-output-format"); args.push_back("mp4");
        args.push_back("-o"); args.push_back(output_template);
        args.push_back("--no-warnings"); args.push_back("--newline");
        args.push_back("--progress"); args.push_back("--no-playlist");
        args.push_back("--continue");            // resume from breakpoint (continue .part)
        args.push_back("--retries"); args.push_back("20");
        args.push_back("--fragment-retries"); args.push_back("20");
        args.push_back(url);

        {
            std::string args_dbg;
            for (auto& a : args) args_dbg += a + " ";
...
bool App::start_one_download(TreeNodePtr n) {
        ...
        if (existing.empty()) existing = CacheManager::instance().get_local_file(n->url);
        ...
        std::string dl_id = ProgressManager::instance().start_download(
            n->title, n->url, is_youtube, &sr);
        ...
        if (is_youtube) {
            ...
            pool_.submit([this, url = n->url, base_name, dir, title = n->title, dl_id, n]() {
                ytdlp_download(url, YouTubeChannelParser::ytdlp_youtube_args(),
                               dir, base_name, title, dl_id, n);
            });
            ...
            pool_.submit([this, url = n->url, base_name, dir, title = n->title, dl_id, n, url_type]() {
                ...
                std::error_code ec;
                if (!cf.empty() && fs::exists(cf, ec)) { site.push_back("--cookies"); site.push_back(cf); }
                ...
                pool_.submit([this, url = n->url, dir, base_name, title = n->title, dl_id, is_video_file, n]() {
                    ...
                    if (fs::exists(filepath, ec_fs)) {
                        ...
                        if (!ec_fs && fsize > 0) {
                            bool garbage = false;
                            {
                                ...
                                char c = 0;
                                if (head.read(&c, 1)) {
                                    if (c == '<' || c == '{') garbage = true;  // HTML/XML/JSON error page
                                }
                            }
                            if (garbage) {
                                std::error_code rc; fs::resize_file(filepath, 0, rc);
                                EVENT_LOG(fmt::format("Discarding garbage partial (re-downloading from start): {}", title));
                            } else {
                                resume_from = static_cast<curl_off_t>(fsize);
                                EVENT_LOG(fmt::format("Resuming {} from {} bytes", title, (long long)fsize));
                            }
                        }
                    }
                    ...
                    if (curl && f) {
                        ...
                        progress_data.resume_offset = resume_from;  // overall percentage includes the existing part
                        ...
                        for (int attempt = 1; attempt <= MAX_RETRIES && url_safe; ++attempt) {
                            ...
                            ProgressManager::instance().update(
                                dl_id, 0,
                                attempt > 1 ? fmt::format("Retry {}/{}", attempt, MAX_RETRIES) : "Connecting...",
                            ...
                            if (perform_rc == CURLE_OK && http_code == 416 && resume_from > 0) {
                                LOG(fmt::format("[DL] HTTP 416 (range not satisfiable) — file already complete: {}", title));
                                success = true;
                                break;
                            }
                            ...
                            if (perform_rc == CURLE_OK && http_code >= 200 && http_code < 300) {
                                ...
                                if (resume_from > 0 && http_code == 200) {
                                    ...
                                    progress_data.last_bytes = 0;
                                    LOG(fmt::format("[DL] server ignored range (200), restart from 0: {}", title));
                                    continue;
                                }
                                success = true;
                                break;
                            ...
                            bool transient = (perform_rc == CURLE_OPERATION_TIMEDOUT ||
                                              perform_rc == CURLE_COULDNT_CONNECT ||
                                              perform_rc == CURLE_COULDNT_RESOLVE_HOST ||
                                              perform_rc == CURLE_PARTIAL_FILE ||
                                              perform_rc == CURLE_RECV_ERROR ||
                                              perform_rc == CURLE_SEND_ERROR ||
                                              perform_rc == CURLE_GOT_NOTHING ||
                                              perform_rc == CURLE_SSL_CONNECT_ERROR ||
                                              (http_code >= 500 && http_code < 600));
                            LOG(fmt::format("[DL] attempt {}/{} rc={} http={} transient={} : {}",
                                            attempt, MAX_RETRIES, (int)perform_rc, http_code, transient, title));
                        ...
                        if (!success) {
                            EVENT_LOG(fmt::format("Download failed after retries, partial kept for resume: {}", title));
                        }
                    } else {
                        if (f) fclose(f);
    ...
    void App::pump_download_queue(size_t current_slot_count) {
        while (current_slot_count < (size_t)MAX_CONCURRENT_DOWNLOADS && !pending_downloads_.empty()) {
            ...
            if (start_one_download(n)) ++current_slot_count;
        }
    }
