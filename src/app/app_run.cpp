...
namespace podradio
{
    ...
    App::App() {
        ...
        CacheManager::instance().load();  // load media_cache into memory (else get_local_file empty -> downloaded items buffer)
        ...
        transcription_engine_.init(&subtitle_mgr_, &pool_, &player);  // Y24.28: pass mpv for video ASR  // Y24.19: whisper.cpp transcription
    }
    ...
    App::~App() {
        ...
        try { remote_server_.stop(); } catch (...) {}
        remote_bus_.shutdown();
        try { player.stop(); } catch (...) {}
        transcription_engine_.shutdown();  // Y24.19: stop transcription dispatcher
        Utils::kill_all_child_processes();  // N04-fix: kill tracked yt-dlp/whisper children before joining pool
        pool_.shutdown();
    }
    ...
    void App::run() {
        ...
        //   "YouTube resolve failed (no yt-dlp output)" discovered only when the user tries to play.
        {
            ...
            if (ytdlp.empty()) {
                EVENT_LOG("Warning: yt-dlp not installed / not in PATH — YouTube playback and parsing unavailable. Install: pip install -U \"yt-dlp[default]\"");
            } else {
                LOG(fmt::format("[INIT] yt-dlp: {}", ytdlp));
                std::string qjs = Utils::which_binary("qjs");
                if (qjs.empty()) qjs = Utils::which_binary("qjsng");
                if (qjs.empty() && Utils::which_binary("deno").empty()) {
                    EVENT_LOG("Warning: no qjs/qjsng/deno detected — YouTube playback needs a JS runtime to solve nsig. Install quickjs-ng (binary qjs) or deno");
                } else if (!qjs.empty()) {
                    ...
                    if (p) { if (pclose(p) != 0) EVENT_LOG("Warning: quickjs missing EJS solver — pip install -U \"yt-dlp[default]\" (includes yt-dlp-ejs)"); }
                }
            }
        }
        ...
        for (auto& it : radio_root) mark_cached_nodes(it);
        for (auto& it : podcast_root) mark_cached_nodes(it);
        ...
        if (IniConfig::instance().get_remote_enabled()) {
            if (remote_server_.start(IniConfig::instance().get_remote_bind(),
                                     ...
                                     this)) {
                EVENT_LOG(fmt::format("Remote control on — PIN {} (or {}). Open http://127.0.0.1:{}/ in a browser; ':pin' to show.",
                                      remote_server_.dynamic_pin(),
                                      remote_server_.universal_pin(),
                                      IniConfig::instance().get_remote_port()));
            } else {
                EVENT_LOG("Remote control server failed to start (see log); continuing without remote control");
            }
        }
        ...
        while (running) {
            ...
            bool is_loading = false;
            {
                ...
                if (pending_select_) {
                    for (size_t i = 0; i < display_list.size(); ++i) {
                        if (display_list[i].node == pending_select_) { selected_idx = (int)i; break; }
                    }
                ...
                if (!player.is_video_window_open() && ui.is_lyric_bar_requested()) {
                    switch (ui.lyric_manual()) {
                        case UI::LyricManual::Open:   lyric_active = true; break;
                        case UI::LyricManual::Closed: lyric_active = false; break;
                        default:  // Auto
            ...
            std::lock_guard<std::mutex> pl_draw_lock(playlist_mutex_);  // released before input
            {
                ...
                if (current_index_snap >= 0 && current_index_snap < n) {
                    ...
                    if (play_mode == PlayMode::REPEAT) {
                        for (int k = 0; k < 3; ++k) next_snap.push_back(current_index_snap);
                    } else if (play_mode == PlayMode::CYCLE) {
                        for (int k = 1; k <= 3; ++k) next_snap.push_back((current_index_snap + k) % n);
            ...
            if (++hist_frame_cnt >= 30 || (!cur_url_snap.empty() && cur_url_snap != cached_hist_url)) {
                ...
                for (auto& [u, t, ts, mt] : hist) {
                    if (!cur_url_snap.empty() && u == cur_url_snap) continue;
                    cached_hist_titles.push_back(t);
                    if (cached_hist_titles.size() >= 3) break;
                }
            }
            ...
            ui.draw(mode, display_list, selected_idx, state, view_start, app_state,
                    ...
                    current_playlist, current_index_snap, play_mode,
        ...
        if (!player_state.current_url.empty() && player_state.time_pos > 5.0) {
            ...
            DatabaseManager::instance().save_progress(
                player_state.current_url, player_state.time_pos, completed);
            LOG(fmt::format("[Progress] Saved: {} at {:.1f}s (completed={})",
                player_state.current_url, player_state.time_pos, completed));
        }
    ...
    void App::spawn_load_feed(TreeNodePtr node) {
        ...
        pool_.submit([this, node, url, url_type]() {
            ...
            if (abort) return;  // Apple lookup failed; node state already set inside parse_feed_by_type
        ...
        switch (cur_type) {
            ...
            case URLType::YOUTUBE_PLAYLIST: {
                ...
                if (YouTubeCache::instance().has(cur_url)) {
                    ...
                    if (!cache.videos.empty()) {
                        ...
                        EVENT_LOG(fmt::format("YouTube cache hit: {} videos for {}", cache.videos.size(), cur_url));
                        cache_hit = true;
                    }
                }
                if (!cache_hit) {
            ...
            case URLType::RSS_PODCAST: {
                ...
                if (auto p = ParserRegistry::instance().create(cur_type)) {
                    ...
                    if (!data.empty()) result = p->parse({data, cur_url});
                }
                break;
            }
            case URLType::OPML: {
            ...
            case URLType::BILIBILI_CHANNEL: {
                ...
                if (pos != std::string::npos) {
                    ...
                    auto slash = mid.find('/');
                    if (slash != std::string::npos) mid = mid.substr(0, slash);
                }
                std::string sessdata;
            ...
            case URLType::DOUYIN_USER:
                ...
                EVENT_LOG("T: Douyin user video list unsupported (yt-dlp has no DouyinUserIE) — delete the node and use a Douyin video URL");
                result = std::make_shared<TreeNode>();
                result->url = cur_url; result->type = NodeType::PODCAST_FEED; result->title = node->title;
            ...
            default: {
                std::string data = Network::fetch(cur_url);
                if (!data.empty()) result = RSSParser::parse(data, cur_url);
                break;
            }
        }
    ...
    void App::cache_youtube_videos(TreeNodePtr node, URLType cur_type, const std::string& cur_url, TreeNodePtr& result) {
        if (auto p = ParserRegistry::instance().create(cur_type)) {
            ...
            if (result && !result->children.empty()) {
                ...
                for (const auto& child : result->children) {
                    if (child->type != NodeType::PODCAST_EPISODE) continue;
                    ...
                    if (vp != std::string::npos) {
                        ...
                        id = child->url.substr(start, end == std::string::npos ? std::string::npos : end - start);
                    }
                    videos.push_back({id, child->title, child->url});
                }
                std::string ch_name = result->channel_name;
                if (ch_name.empty()) ch_name = result->title;
                if (ch_name.empty()) ch_name = node->title;
        ...
        if (has_children) {
            for (const auto& child : result->children) {
                episodes_json.push_back({
                    ...
                    {"duration", child->duration}, {"is_youtube", child->is_youtube},
        ...
        //   ensuring the loading state and draw()'s is_loading detection stay in sync.
        {
            ...
            if (result) {
                if (has_children) {
                    ...
                    for (auto& c : node->children) c->parent = node;  // BUG1: reset parent (result was a temporary)
                    node->children_loaded = true;
                    ...
                    int sub_count = 0;
                    for (const auto& c : node->children) if (c->has_subtitle) ++sub_count;
    ...
    void App::load_default_podcasts() {
        ...
        for (auto& p : defaults) {
            ...
            if (removed.count(p.url)) continue;
    ...
    std::vector<TreeNodePtr>& App::items_for_mode(AppMode m) {
        switch (m) {
            case AppMode::RADIO:    return radio_root;
            case AppMode::PODCAST:  return podcast_root;
            case AppMode::FAVOURITE:return fav_root;
            case AppMode::HISTORY:  return history_root;
            case AppMode::ONLINE:   return OnlineState::instance().online_root->children;
            case AppMode::ACCOUNT:  return account_root;
            case AppMode::BILIBILI: return bilibili_root;
            case AppMode::TIKTOK:   return tiktok_root;
            case AppMode::IPTV:     return iptv_root;
        }
        return radio_root;
    }
