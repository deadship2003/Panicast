...
static std::string iptv_cache_path(const std::string& url) {
    std::string key = url;
    for (char& c : key) if (!isalnum((unsigned char)c) && c != '.' && c != '-') c = '_';
    if (key.size() > 128) key = key.substr(key.size() - 128);
...
static bool is_sub_playlist(const std::string& url) {
    ...
    if (p.size() >= 4 && p.compare(p.size() - 4, 4, ".m3u") == 0) {
        if (p.size() >= 5 && p.compare(p.size() - 5, 5, ".m3u8") == 0) return false;  // HLS stream
        return true;
    }
    return false;
}
...
static std::vector<TreeNodePtr> build_channel_tree(const std::vector<IptvChannel>& ch) {
    ...
    if (ch.empty()) return kids;
    std::map<std::string, std::vector<IptvChannel>> by_group;
    for (const auto& c : ch) by_group[c.group.empty() ? "Other" : c.group].push_back(c);
    if (by_group.size() == 1) {
        for (const auto& c : ch) kids.push_back(make_iptv_node(c));
    } else {
...
static std::vector<TreeNodePtr> build_catalog(const std::string& body, const std::string& kind,
                                              const std::string& code_key, const std::string& name_key) {
    std::vector<TreeNodePtr> kids;
    if (body.empty()) return kids;
    try {
        auto j = json::parse(body);
        if (!j.is_array()) return kids;
        for (const auto& e : j) {
...
void App::load_iptv_root() {
    ...
    if (!iptv_root.empty()) return;  // catalog already built
...
void App::expand_iptv_node(TreeNodePtr node) {
    ...
    if (u.empty() || u.rfind("iptv:", 0) != 0) return;
    ...
    pool_.submit([this, node, u]() {
        ...
        if (u == "iptv:all") {
            ...
            kids = build_channel_tree(parse_m3u(iptv_fetch(base + "/regions/" + to_lower(u.substr(12)) + ".m3u")));
            ...
            kids = build_channel_tree(parse_m3u(iptv_fetch(base + "/countries/" + to_lower(u.substr(13)) + ".m3u")));
            ...
            kids = build_channel_tree(parse_m3u(iptv_fetch(base + "/categories/" + u.substr(14) + ".m3u")));
            ...
            kids = build_channel_tree(parse_m3u(iptv_fetch(base + "/languages/" + to_lower(u.substr(14)) + ".m3u")));
            ...
            for (; it != end; ++it) {
                ...
                if (cu.empty() || cu.rfind("http", 0) != 0) continue;
                auto n = std::make_shared<TreeNode>();
                // Use the last URL path segment as a label, or the whole URL.
                size_t slash = cu.find_last_of('/');
                n->title = (slash != std::string::npos && slash + 1 < cu.size()) ? cu.substr(slash + 1) : cu;
                ...
                std::string meta = Paths::get_data_dir() + "/iptv-cache/custom_" + std::to_string(idx) + ".url";
                std::error_code ec;
                fs::create_directories(fs::path(meta).parent_path(), ec);
                std::ofstream f(meta); if (f) f << cu;
                kids.push_back(n);
                ++idx;
            }
            if (kids.empty()) {
            ...
            std::string meta = Paths::get_data_dir() + "/iptv-cache/custom_" + u.substr(15) + ".url";
            std::ifstream f(meta);
            std::string cu((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>());
            // strip whitespace
            while (!cu.empty() && (cu.back() == '\n' || cu.back() == '\r' || cu.back() == ' ')) cu.pop_back();
            if (!cu.empty()) kids = build_channel_tree(parse_m3u(iptv_fetch(cu)));
        }
