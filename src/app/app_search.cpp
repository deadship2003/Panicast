...
using json = nlohmann::json;
    ...
    void App::perform_search() {
        ...
        std::string ql = Utils::to_lower(q);
        {
            ...
            for (auto& it : cur_items()) search_recursive(it, ql, search_matches);
            ...
            for (auto& m : search_matches) {
                if (seen.insert(m).second) unique.push_back(m);
            }
    ...
    void App::reveal_node(TreeNodePtr node) {
        ...
        std::lock_guard<std::recursive_mutex> lock(tree_mutex);  // traversal + changing expanded must be mutually exclusive
        for (auto& it : cur_items()) reveal(it);  // E: was reveal(current_root)
    }
    ...
    TreeNodePtr App::make_search_history_child(TreeNodePtr account_node, const std::string& source, int account_id) {
    ...
    void App::finalize_search(const std::string& source, int account_id, TreeNodePtr account_node,
        ...
        for (const auto& elem : results_json) nodes.push_back(build_search_result_node(source, elem));
        add_search_record(source, account_id, account_node, query, "", nodes);  // P2 (Y23.7): keyword-only cache
    }
    ...
    TreeNodePtr App::build_search_result_node(const std::string& source, const json& r) {
        ...
        if (c->art_url.empty()) c->art_url = r.value("upic", "");
        if (c->art_url.empty()) c->art_url = r.value("pic", "");

        if (source == "youtube") {
            c->is_youtube = true;
            c->title = r.value("title", "Untitled");
            c->channel_name = r.value("channel_title", "");
            if (!c->channel_name.empty() && kind != "channel") c->subtext = c->channel_name;
            if (kind == "channel") {
                c->is_yt_channel = true; c->channel_id = r.value("id", "");
                c->type = NodeType::FOLDER; c->children_loaded = false;
            } else if (kind == "playlist") {
                c->is_yt_channel = true; c->is_yt_playlist = true; c->channel_id = r.value("id", "");
                c->type = NodeType::FOLDER; c->children_loaded = false;
            } else {  // video or music
                c->type = NodeType::PODCAST_EPISODE; c->children_loaded = true;
                if (kind == "music") c->is_yt_music = true;
            }
        } else {  // bilibili
            c->is_youtube = false;
            if (kind == "up") {
                c->is_yt_channel = true; c->is_bili_up = true; c->channel_id = r.value("mid", "");
                c->channel_name = r.value("uname", "");
                int fans = r.value("fans", 0);
                c->title = fans > 0 ? fmt::format("{} ({} fans)", c->channel_name, fans) : c->channel_name;
    ...
    void App::load_search_record_children(TreeNodePtr node, const std::string& source) {
        if (!node || node->url.rfind("search:" + source + ":", 0) != 0) return;
        size_t qstart = 7 + source.size() + 1;
        if (qstart >= node->url.size()) return;
        std::string query = node->url.substr(qstart);
        std::string cached = DatabaseManager::instance().load_search_cache_src(source, node->account_id, query);
        ...
        try {
            ...
            for (const auto& r : j["results"]) {
                ...
                if (c) { c->parent = node; node->children.push_back(c); }
            }
            node->children_loaded = true;
            node->expanded = true;
    ...
    void App::expand_search_history(TreeNodePtr node) {
        if (!node || !node->is_search_parent) return;
        std::string source;
        if (node->url.rfind("searchhist:", 0) == 0) source = node->url.substr(11);
        ...
        node->children_loaded = true;
        node->expanded = true;
        EVENT_LOG(fmt::format("Loaded {} search records [{}/{}]", items.size(), source, node->account_id));
    }
    ...
    void App::add_search_record(const std::string& source, int account_id, TreeNodePtr account_node,
        ...
        std::lock_guard<std::recursive_mutex> lock(tree_mutex);
        TreeNodePtr sp;
        for (auto& c : account_node->children) if (c->is_search_parent) { sp = c; break; }
        if (!sp) {
        ...
        sp->children.erase(std::remove_if(sp->children.begin(), sp->children.end(),
            [&](const TreeNodePtr& n){ return n->is_yt_search && n->url == rec_url; }), sp->children.end());
        auto rec = std::make_shared<TreeNode>();
        ...
        rec->expanded = false;
        rec->parent = sp;
        for (auto& r : result_nodes) { r->parent = rec; rec->children.push_back(r); }
    ...
    void App::rerun_search_record(TreeNodePtr node) {
        if (!node || !node->is_yt_search || node->url.rfind("search:", 0) != 0) return;
        ...
        if (source == "youtube") perform_youtube_search(query);
        else if (source == "bilibili") perform_bilibili_search(query);
    }
    ...
    void App::delete_search_record(TreeNodePtr node) {
        ...
        if (p) {
            std::lock_guard<std::recursive_mutex> lock(tree_mutex);
            p->children.erase(std::remove_if(p->children.begin(), p->children.end(),
                [&](const TreeNodePtr& n){ return n.get() == node.get(); }), p->children.end());
        }
