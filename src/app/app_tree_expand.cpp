...
namespace panicast
{
    ...
    std::vector<TreeNodePtr>* App::get_root_by_mode_string(const std::string& mode_str) {
        ...
        if (mode_str == "ONLINE") return &OnlineState::instance().online_root->children;
    ...
    void App::sync_link_node_status(TreeNodePtr target) {
        ...
        for (auto& it : fav_root) sync_children(it);
        for (auto& it : podcast_root) sync_children(it);
    }
    ...
    bool App::expand_link_node(TreeNodePtr node) {
        ...
        if (!target) {
            ...
            // Normal node: look up in the corresponding root
                ...
                for (auto& it : *search_root) { target = find_node_by_url(it, target_url); if (target) break; }
            }
        }
            ...
            if (target->children_loaded && !target->children.empty()) {
                // Real node already loaded; sync children directly to the LINK
                {
                    ...
                    for (auto& child : target->children) {
                        node->children.push_back(child);  // do not reset parent: child is shared with target; resetting would break target's parent-pointer invariant
                    }
                }
            ...
            if (DatabaseManager::instance().is_episode_cached(target_url)) {
                ...
                if (!episodes.empty()) {
                    ...
                    // Sync to the LINK node
                    {
                        ...
                        node->children.clear();
                        for (auto& child : target->children) {
                            node->children.push_back(child);  // do not reset parent: see above, consistent with sync_link_node_status
                        }
                    }
                    node->children_loaded = true;
                    node->expanded = true;
        ...
        if (target_url == "online_root") {
            ...
            }
            {
                ...
                for (auto& child : OnlineState::instance().online_root->children) {
                    node->children.push_back(child);  // do not reset parent: online_root children are shared; resetting breaks their parent-pointer invariant
                }
            }
            node->children_loaded = true;
            node->expanded = true;
    ...
    void App::expand_local_folder(TreeNodePtr node) {
        ...
        for (fs::directory_iterator it(dir, ec), end; it != end; it.increment(ec)) {
            ...
            if (!is_media_extension(ext)) continue;
            auto child = std::make_shared<TreeNode>();
            child->title = it->path().filename().string();
            child->url = it->path().string();    // absolute path; classify and MPV can both handle it directly
                  ...
                  [](const TreeNodePtr& a, const TreeNodePtr& b){ return a->title < b->title; });
        {
            std::lock_guard<std::recursive_mutex> lock(tree_mutex);
            node->children.clear();
            for (auto& c : subdirs) node->children.push_back(c);
            for (auto& c : files) node->children.push_back(c);
            node->children_loaded = true;
            node->expanded = true;
        }
        ...
        if (!files.empty()) {
            pool_.submit([files]() {
                for (auto& c : files) SubtitleManager::probe_sidecar(c);
            });
        }
        size_t total = subdirs.size() + files.size();
        if (total == 0) {
            EVENT_LOG(fmt::format("[LocalFolder] Directory empty (no audio/video files/subfolders): {}", dir));
        } else {
            EVENT_LOG(fmt::format("[LocalFolder] Loaded {} items ({} subfolders, {} files): {}",
                                  total, subdirs.size(), files.size(), dir));
        }
    }
