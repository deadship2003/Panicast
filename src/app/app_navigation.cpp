...
namespace panicast
{
    ...
    void App::jump_to_playing() {
        ...
        switch (playback_mode_) {
            ...
            case AppMode::ONLINE:    break;
            case AppMode::ACCOUNT:   break;
            case AppMode::BILIBILI:  break;
            case AppMode::TIKTOK:    break;
            case AppMode::IPTV:      break;
        }
        reset_search();
        selected_idx = 0;
        // Expand all ancestors so the node appears in the flattened display_list.
        {
            ...
            while (p) { p->expanded = true; p = p->parent.lock(); }
        }
    ...
    void App::nav_page_down() {
        ...
        if (display_list.empty()) { selected_idx = 0; return; }
        if (selected_idx >= (int)display_list.size()) selected_idx = (int)display_list.size() - 1;
    }
            ...
            if (need_load) {
                if (node->is_search_parent) {
                    ...
                    if (node->type == NodeType::PODCAST_FEED &&
                        ...
                        if (!episodes.empty()) {
                            build_episode_children_from_cache(node, episodes);  // Y24.29: was inline (missing has_asr_srt)
    ...
    void App::toggle_sort_order() {
        ...
        bool reversed = false; int n = 0; std::string scope_desc;
        {
            std::lock_guard<std::recursive_mutex> lock(tree_mutex);
            std::vector<TreeNodePtr>* targets = sort_top ? &cur_items() : &parent->children;
            n = (int)targets->size();
            if (n == 0) { EVENT_LOG("Sort: No siblings to sort"); return; }
            if (sort_top) { cur_sort_reversed = !cur_sort_reversed; reversed = cur_sort_reversed; }
            else { parent->sort_reversed = !parent->sort_reversed; reversed = parent->sort_reversed; }
            if (reversed) {
                std::sort(targets->begin(), targets->end(),
                    [](const TreeNodePtr& a, const TreeNodePtr& b) { return title_compare_desc(a->title, b->title); });
            } else {
                std::sort(targets->begin(), targets->end(),
                    [](const TreeNodePtr& a, const TreeNodePtr& b) { return title_compare_asc(a->title, b->title); });
            }
            display_list.clear();
