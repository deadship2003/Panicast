...
namespace panicast
{
    ...
    void App::switch_mode(AppMode new_mode) {
        ...
        switch (new_mode) {
            case AppMode::RADIO: break;
            case AppMode::PODCAST: break;
            case AppMode::FAVOURITE: break;
            case AppMode::HISTORY: break;
            case AppMode::ONLINE:
                OnlineState::instance().load_search_history();
                break;
            case AppMode::ACCOUNT: load_accounts_root(); break;
            case AppMode::BILIBILI: load_bilibili_root(); break;
            case AppMode::TIKTOK: load_tiktok_root(); break;
            case AppMode::IPTV: load_iptv_root(); break;
        }
        reset_search();
        selected_idx = 0;
    }
    ...
    void App::handle_mouse_event() {
        ...
        // Wheel paging
        if (ev.bstate & BUTTON4_PRESSED) { nav_page_up(); return; }
        if (ev.bstate & BUTTON5_PRESSED) { nav_page_down(); return; }

        // Left click: only respond to clicks in the left-panel content area
        if (!(ev.bstate & (BUTTON1_CLICKED | BUTTON1_PRESSED))) return;
        int lw = ui.get_left_w();
        int th = ui.get_top_h();
        if (lw <= 0 || th <= 0) return;
        // Content row range: y in [1, top_h-2] (0 = border title, top_h-1 = bottom border)
        if (ev.y < 1 || ev.y >= th - 1) return;
        if (ev.x < 0 || ev.x >= lw) return;
        ...
        if (now - last_click < std::chrono::milliseconds(250)) return;
        last_click = now;

        int row = view_start + (ev.y - 1);
        if (row < 0 || row >= (int)display_list.size()) return;
        selected_idx = row;
    ...
    void App::configure_proxy() {
        std::string cur = IniConfig::instance().get_proxy();  // normalized currently-effective proxy (empty = none)
        std::string input = ui.input_box(
            "Proxy (empty = direct connection)", cur, /*prefill=*/true);
        if (UI::is_input_cancelled(input)) { EVENT_LOG("Proxy config cancelled"); return; }
    ...
    void App::configure_cookies() {
        ...
        if (mode == AppMode::BILIBILI) {
            section = "bilibili"; key = "cookies_file"; label = "Bilibili"; def_name = "bilibili_cookie.txt";
            cur = IniConfig::instance().get_bilibili_cookies_file();
        } else if (mode == AppMode::TIKTOK) {
            bool cn = (tiktok_region_ == "CN");
            section = "tiktok";
            if (cn) { key = "douyin_cookies_file"; label = "Douyin"; def_name = "douyin_cookie.txt"; cur = IniConfig::instance().get_tiktok_douyin_cookies_file(); }
            else    { key = "cookies_file";        label = "TikTok"; def_name = "tiktok_cookie.txt";  cur = IniConfig::instance().get_tiktok_cookies_file(); }
        } else {
            section = "youtube"; key = "cookies_file"; label = "YouTube"; def_name = "youtube_cookie.txt";
            cur = IniConfig::instance().get_youtube_cookies_file();
        }
        std::string input = ui.input_box(
            label + " cookies.txt path (bare/empty = <data_dir>/" + def_name + "; or absolute)", cur, /*prefill=*/true);
        if (UI::is_input_cancelled(input)) { EVENT_LOG("Cookies config cancelled"); return; }
    ...
    void App::open_command_window() {
        std::string input = ui.input_box(
            "Command (r/s/c; mpv hotkey — see ? for full list)", "", /*prefill=*/false);
        if (UI::is_input_cancelled(input)) { EVENT_LOG(": command cancelled"); return; }
        ...
        else if (s.size() >= 2 && std::string("shuffle").rfind(s, 0) == 0) new_mode = PlayMode::SHUFFLE;
        else if (s.size() >= 2 && std::string("cycle").rfind(s, 0) == 0) new_mode = PlayMode::CYCLE;
        else {
            EVENT_LOG(fmt::format(": unknown play mode '{}' (try :re/:sh/:cy or :repeat/:shuffle/:cycle)", input));
            return;
        }
    ...
    void App::handle_input(int ch, int marked_count) {
        ...
        if (ch == 25) {  // ASCII 25 = Ctrl+Y
            ...
            if (cn && !cn->url.empty()) url = cn->url;                         // cursor object (preferred)
            else if (playback_node && !playback_node->url.empty()) url = playback_node->url;  // fallback
            if (url.empty()) {
            ...
            if (cn && cn->has_subtitle && !cn->subtitle_url.empty())
                LOG(fmt::format("[Clipboard] cursor '{}' has transcript URL: {}", cn->title, cn->subtitle_url));
            return;
        }
        ...
        if (visual_mode_) {
            ...
            else if (ch == 'V') { visual_mode_ = false; clear_all_marks(); }  // uppercase V cancels all
        ...
        switch (ch) {
            ...
            case 27:    // ESC key
                ...
                break;
            case '?': ui.show_help(player.get_state()); break;  // show help popup directly
            case 'R': switch_mode(AppMode::RADIO); break;
            case 'P': switch_mode(AppMode::PODCAST); break;
            case 'F': switch_mode(AppMode::FAVOURITE); break;
            case 'H': switch_mode(AppMode::HISTORY); break;
            case 'O': {  // ONLINE mode
            ...
            case 'r': {
                ...
                if (rn && rn->is_account && mode == AppMode::ACCOUNT) {
                    ...
                    reset_search(); refresh_node();     // feeds / channels / T creators / etc.
                }
                break;
            }
            ...
            case '\n': {  // Enter/l: play the selected episode (its peers become the list); expand folders
                ...
                if (mode == AppMode::BILIBILI && selected_idx >= 0 && selected_idx < (int)display_list.size()) {
                    auto bn = display_list[selected_idx].node;
                    if (bn && bn->is_account) { expand_bilibili_account(bn); break; }
                    ...
                    if (bn && bn->is_bili_followings && !bn->children_loaded) { expand_bili_followings(bn); break; }
                    if (bn && bn->is_bili_history && !bn->children_loaded) { expand_bili_history(bn); break; }
            ...
            case KEY_BACKSPACE:
            case '\\': player.reset_speed(); break;  // backslash key also supports speed reset
            ...
            case 'a': {
                ...
                if (mode == AppMode::BILIBILI) {
                    if (selected_idx >= 0 && selected_idx < (int)display_list.size()) {
                        auto n = display_list[selected_idx].node;
                        if (n && n->is_yt_search_result && n->is_yt_channel && !n->is_youtube && !n->channel_id.empty()) {
            ...
            case 'e': edit_node(); break;  // edit node title and URL
            case 'f': {
            ...
            case 'V': clear_all_marks(); break;  // uppercase V cancels all selections
            case 'C': clear_playlist(); break;   // N04-fix: clear peer playlist, keep playing
            case 'T': {  // Y24.11: T mode (TikTok/Douyin) — was tree-lines toggle (now default ON, no keybind)
                switch_mode(AppMode::TIKTOK);
                EVENT_LOG(fmt::format("T mode (TikTok, region {})", TikTokRegion::name(current_tiktok_region())));
                break;
            }
            ...
            case 'L': {  // Y24.43: unified subtitle flow (local-first; L never stops ASR)
                ...
                if (pst.has_media) {
                    ...
                    auto find_local_srt = [&]() -> std::string {
                        ...
                        if (!local.empty()) {
                            ...
                            if (dot != std::string::npos) lf_base = lf_base.substr(0, dot);
                            srt = lf_base + ".srt";
                        }
                        if (std::filesystem::exists(srt)) {
                            if (!local.empty()) playback_node->local_file = local;
                            return srt;
                        }
                        return "";
                    };
                    ...
                    if (vo_open) {
                        ...
                        if (player.has_active_subtitle()) {
                            EVENT_LOG("L: embedded subtitle present (English via slang) - no ASR, mpv renders in video window");
                            break;
                        }
                        ...
                        if (playback_node && playback_node->has_subtitle && !playback_node->subtitle_url.empty()) {
                            player.sub_add(playback_node->subtitle_url);
                            EVENT_LOG("L: online transcript -> video window");
                            break;
                        }
                        transcription_engine_.start_realtime(playback_node, url, is_streaming, /*is_video=*/true);
                    ...
                    transcription_engine_.start_realtime(playback_node, url, is_streaming, /*is_video=*/false);
                    break;
                }
                ...
                if (mode == AppMode::FAVOURITE) {
                    ...
                    if (marked_count > 0) {
                        ...
                        auto n = display_list[selected_idx].node;
                        bool is_local = !n->local_file.empty() ||
                                        (!n->url.empty() &&
                                         (n->url[0] == '/' || n->url.rfind("file://", 0) == 0));
            ...
            case 'U': {  // toggle icon style (ASCII/Emoji)
                ...
                EVENT_LOG(fmt::format("Icon style: {} (saved to config)",
                    IconManager::get_style_name()));
                break;
            }
            case 12:  // Ctrl+L - cycle through 22 color themes
                ui.toggle_theme();
                break;
            case 'o': toggle_sort_order(); break;     // toggle sort order
            case '/':
                if (mode == AppMode::ONLINE) {
