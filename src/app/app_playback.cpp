...
void App::load_transcript(TreeNodePtr node) {
    subtitle_mgr_.load_async(node, pool_);
}
void App::probe_local_sidecar(TreeNodePtr node) {
    subtitle_mgr_.probe_sidecar(node);
}
...
void App::play_current(int idx) {
    transcription_engine_
        .stop_realtime(); // Y24.20: realtime transcription doesn't carry across tracks
    std::string orig_url;
