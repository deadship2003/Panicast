...
bool URLClassifier::ends_with_ci(const std::string& s, const char* suffix) {
    size_t slen = s.size(), n = 0; while (suffix[n]) ++n;
    if (n == 0 || slen < n) return false;
...
URLType URLClassifier::classify(const std::string& url) {
    ...
    if (url.compare(0, 7, "file://") == 0 || url[0] == '/') {
        ...
        for (auto e : vid_ext) {
            if (ends_with_ci(lpath, e)) return URLType::VIDEO_FILE;
        }
    ...
    if (url.find("tiktok.com") != std::string::npos) {
        if (url.find("/video/") != std::string::npos) return URLType::TIKTOK_VIDEO;
        if (url.find("/@") != std::string::npos) return URLType::TIKTOK_USER;
    }
    ...
    for (const auto& p : PATTERNS) {
        if (p.suffix) {
            if (ends_with_ci(path, p.needle)) return p.type;
        } else {
...
std::string URLClassifier::type_name(URLType type) {
    switch (type) {
        ...
        case URLType::YOUTUBE_PLAYLIST: return "YouTube Playlist";
        ...
        case URLType::BILIBILI_CHANNEL: return "Bilibili Channel";
        case URLType::BILIBILI_VIDEO: return "Bilibili Video";
        case URLType::DOUYIN_USER: return "Douyin User";
        case URLType::DOUYIN_VIDEO: return "Douyin Video";
        case URLType::TIKTOK_USER: return "TikTok User";
        case URLType::TIKTOK_VIDEO: return "TikTok Video";
...
bool URLClassifier::is_video(URLType type) {
    ...
    return is_youtube(type) || type == URLType::VIDEO_FILE
        || type == URLType::BILIBILI_VIDEO || type == URLType::DOUYIN_VIDEO
        || type == URLType::TIKTOK_VIDEO;
}
...
MediaType URLClassifier::classifyMediaType(const std::string& url) {
    if (url.empty()) return MediaType::Radio;
    ...
    if (url.compare(0, 7, "file://") == 0 || url[0] == '/') {
        return (classify(url) == URLType::VIDEO_FILE) ? MediaType::LocalVideo : MediaType::LocalAudio;
    }
    ...
    if (url.compare(0, 5, "iptv:") == 0) return MediaType::Iptv;
    std::string path = url_path(url);
    if (ends_with_ci(path, ".m3u8") || ends_with_ci(path, ".m3u")) return MediaType::Iptv;
    ...
    switch (classify(url)) {
        case URLType::YOUTUBE_RSS:    case URLType::YOUTUBE_CHANNEL:
        case URLType::YOUTUBE_VIDEO:  case URLType::YOUTUBE_PLAYLIST:
            return MediaType::Youtube;
        case URLType::BILIBILI_CHANNEL: case URLType::BILIBILI_VIDEO:
            return MediaType::Bilibili;
        // Douyin folds into Tiktok (the CN counterpart; not listable, kept as placeholder).
        case URLType::TIKTOK_USER:    case URLType::TIKTOK_VIDEO:
        case URLType::DOUYIN_USER:    case URLType::DOUYIN_VIDEO:
            return MediaType::Tiktok;
        case URLType::RADIO_STREAM:   case URLType::OPML:
            return MediaType::Radio;
        case URLType::RSS_PODCAST:    case URLType::APPLE_PODCAST:
