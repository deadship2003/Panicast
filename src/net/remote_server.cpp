...
static int make_listen_fd(const std::string& bind_addr, int port)
{
    ...
    if (fd < 0) {  // fallback: IPv4 only
        ...
        if (inet_pton(AF_INET, bind_addr.c_str(), &a4.sin_addr) <= 0) { ::close(fd); return -1; }
        if (::bind(fd, reinterpret_cast<struct sockaddr*>(&a4), sizeof(a4)) < 0) {
            LOG(fmt::format("[REMOTE] bind4({}:{}) failed: {}", bind_addr, port, std::strerror(errno)));
            ::close(fd); return -1;
        }
    }
    if (::listen(fd, 8) < 0) {
...
static std::string peer_ip(const struct sockaddr_storage* ss)
{
    ...
    if (ss->ss_family == AF_INET) {
        inet_ntop(AF_INET, &reinterpret_cast<const struct sockaddr_in*>(ss)->sin_addr, ip, sizeof(ip));
    } else if (ss->ss_family == AF_INET6) {
        inet_ntop(AF_INET6, &reinterpret_cast<const struct sockaddr_in6*>(ss)->sin6_addr, ip, sizeof(ip));
    }
...
bool RemoteServer::start(const std::string& bind_addr, int port, RemoteControlInterface* control)
{
    ...
    EVENT_LOG(fmt::format("Remote control server listening on {}:{} (dual-stack)", bind_addr, port));
    return true;
}
...
void RemoteServer::stop()
{
    ...
    if (ws_accept_thread_.joinable()) ws_accept_thread_.join();  // N06
    if (diff_thread_.joinable()) diff_thread_.join();  // N07
    if (discovery_thread_.joinable()) discovery_thread_.join();  // N05
...
void RemoteServer::accept_loop()
{
    while (running_.load()) {
        ...
        if (fd < 0) {
            if (!running_.load()) break;       // listen socket closed by stop()
...
void RemoteServer::handle_client(int fd, int64_t client_id, bool localhost)
{
    ...
    RemoteSession session(fd, fd, client_id, bus_, control_, /*open_access=*/localhost, dynamic_pin_, universal_pin_);
...
void RemoteServer::notify(const std::string& subsystem){
    ...
    for (RemoteSession* s : snap) {
        if (s) s->notify_change(subsystem);
    }
}
...
void RemoteServer::diff_loop()
{
    ...
    while (running_.load()) {
        ...
        if (!running_.load() || !control_) continue;

        RemoteStateSnapshot s = control_->snapshot_state();
        if (!have_last) { last = s; have_last = true; continue; }

        auto state_str = [](const RemoteStateSnapshot& x) {
            return !x.has_media ? std::string("stop") : (x.paused ? "pause" : "play");
        };
        if (state_str(s) != state_str(last) ||
            s.current_index != last.current_index ||
            s.title != last.title || s.url != last.url ||
            s.elapsed != last.elapsed /* seek */) {
            notify("player");
        }
        if (s.volume != last.volume) notify("mixer");
        if (s.speed != last.speed || s.play_mode != last.play_mode ||
            s.sleep_remaining != last.sleep_remaining) {
            notify("options");
        }
        if (s.mode != last.mode) notify("mode");
        if (s.subtitle_active != last.subtitle_active) notify("subtitle");
        if (s.art_url != last.art_url) notify("art");
...
std::string RemoteServer::regenerate_pin() { return "0000"; }
