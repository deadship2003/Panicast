...
void UI::draw_lyric_content(WINDOW* win, int y_start, int rows, int inner_w, const MPVController::State& state) {
            if (rows <= 0) return;
            if (inner_w < 4) inner_w = 4;
            auto now = std::chrono::steady_clock::now();
            int scroll = (int)std::chrono::duration_cast<std::chrono::milliseconds>(
            ...
            auto draw_row = [&](int row, const std::string& line, bool is_current) {
                ...
                if (is_current) wattron(win, A_BOLD | COLOR_PAIR(11));  // green + bold (theme-adaptive)
                mvwprintw(win, y_start + row, x, "%s", s.c_str());
                if (is_current) wattroff(win, A_BOLD | COLOR_PAIR(11));
            };
            ...
            if (!current_transcript_.empty()) {
                ...
                if (idx >= 0) {
                    auto is_active = [&](int si) -> bool {
                        for (int a : active) if (a == si) return true;
                        return false;
                    };
                    for (int r = 0; r < rows; ++r) {
                        int si = idx - mid + r;  // segment index for this row
                        if (si < 0 || si >= (int)current_transcript_.size()) continue;  // blank
            ...
            if (lyric_history_.empty()) return;
...
void UI::draw_lyric_bar(WINDOW* win, const MPVController::State& state) {
            ...
            werase(win);
            box(win, 0, 0);
            mvwprintw(win, 0, 2, "%s", lyric_bar_title().c_str());  // "🎵 LYRIC" (+ offset) in the top border
            int ww = getmaxx(win);
