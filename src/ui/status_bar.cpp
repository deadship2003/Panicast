...
void UI::draw_status(WINDOW* win, const MPVController::State& state, TreeNodePtr selected_node) {
            ...
            if (now_t != cached_second) {
                char tbuf[32] = {0};
                struct tm tm_local;
                if (std::strftime(tbuf, sizeof(tbuf), "%b %d %Y %H:%M:%S", localtime_local(&now_t, &tm_local)) > 0) {  // thread-safe
            ...
            int left_art_w = Utils::utf8_display_width(ArtRenderer::outer_left()) + Utils::utf8_display_width(ArtRenderer::outer_right());
            int right_art_w = Utils::utf8_display_width(ArtRenderer::right_prefix()) + Utils::utf8_display_width(ArtRenderer::right_suffix());
            ...
            // Middle [] fixed part
            int bracket_fixed_w = Utils::utf8_display_width(ArtRenderer::bracket_left()) + Utils::utf8_display_width(ArtRenderer::bracket_right());

            // Total width needed for full display
            int total_needed = left_art_w + version_w + right_art_w + author_w + bracket_fixed_w + mid_w;

            // Colors
            int left_color = StatusBarColorRenderer::get_color(ThemeManager::instance().statusbar_config(), 0);
            int right_color = StatusBarColorRenderer::get_color(ThemeManager::instance().statusbar_config(), 180);
            int mid_color = StatusBarColorRenderer::get_color(ThemeManager::instance().statusbar_config(), 90);
            ...
            if (total_needed <= inner_w) {
                ...
                left_display = std::string(ArtRenderer::outer_left()) + version_str + ArtRenderer::outer_right();
                mid_display = mid_content.empty() ? "" : std::string(ArtRenderer::bracket_left()) + mid_content + ArtRenderer::bracket_right();
                right_display = std::string(ArtRenderer::right_prefix()) + author_time + ArtRenderer::right_suffix();
                ...
                mid_display = std::string(ArtRenderer::bracket_left()) + "..." + ArtRenderer::bracket_right();
                right_display = std::string(ArtRenderer::right_prefix()) + "..." + ArtRenderer::right_suffix();
                ...
                if (mid_available >= 3) {
                    ...
                    if (mid_w <= mid_available) {
                        mid_display = std::string(ArtRenderer::bracket_left()) + mid_content + ArtRenderer::bracket_right();
                    } else {
                        // Truncate the URL from the middle
                        int inner_bracket_w = mid_available - bracket_fixed_w;
                        mid_display = std::string(ArtRenderer::bracket_left()) + Utils::truncate_middle(mid_content, inner_bracket_w) + ArtRenderer::bracket_right();
                    }
                    left_display = std::string(ArtRenderer::outer_left()) + version_str + ArtRenderer::outer_right();
                    right_display = std::string(ArtRenderer::right_prefix()) + author_time + ArtRenderer::right_suffix();
                    ...
                    if (remaining_for_lr < 4) {
                        ...
                        left_display = std::string(ArtRenderer::outer_left()) + left_version_part + ArtRenderer::outer_right();
                        ...
                        right_display = std::string(ArtRenderer::right_prefix()) + right_author_part + ArtRenderer::right_suffix();
            ...
            if (mid_x < 1) mid_x = 1;  // protect the left border
            ...
            if (need_shrink_mid && mid_display_w > 8) {  // keep at least "[ ... ]"
                int new_mid_w = mid_display_w - total_overflow - 4;  // reduce 4 extra chars to ensure alignment
            ...
            if (left_display_w > left_max_w) {
                if (left_max_w <= 0) {
                    ...
                    if (available_for_version > 0) {
                        left_display = std::string(ArtRenderer::outer_left()) +
            ...
            if (right_display_w > right_max_w) {
                if (right_max_w <= 0) {
                    ...
                    if (available_for_author > 0) {
                        right_display = std::string(ArtRenderer::right_prefix()) +
            ...
            //   the existing window-centering if the gap is too narrow for the middle.
            {
                ...
                if (gap_w >= mid_display_w) {
                    ...
                    if (mid_x < 1) mid_x = 1;
                    if (mid_x + mid_display_w > ww - 1) mid_x = ww - 1 - mid_display_w;
                }
            }
            ...
            if (!mid_display.empty()) {
                ...
                if (url_hyperlink_ && mid_content.rfind("http", 0) == 0) {
                    int sby = 0, sbx = 0; getbegyx(win, sby, sbx);
