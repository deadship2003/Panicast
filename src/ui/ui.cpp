...
void setup_signal_handlers() {
    ...
    for (size_t i = 0; i < sizeof(sigs)/sizeof(int); i++) {
        struct sigaction sa;
        sa.sa_handler = signal_handler;  // sigaction has well-defined semantics (avoids SYSV/BSD differences of signal())
...
void UI::init(float ratio) {
            ...
            if (!codeset_is_utf8(codeset)) {
                ...
                if (fb) { locale_result = fb; codeset = nl_langinfo(CODESET); }
            }
            if (locale_result) {
            ...
            keypad(stdscr, TRUE);
            curs_set(0);
            timeout(30);  //30ms poll (≈33FPS) — snappy input response; heavy work is on pool_ (10 workers), not the main loop
            ...
            mousemask(BUTTON1_CLICKED | BUTTON1_PRESSED | BUTTON4_PRESSED | BUTTON5_PRESSED,
                      nullptr);
            start_color();
            use_default_colors();
            ...
            ThemeManager::instance().statusbar_config() = IniConfig::instance().get_statusbar_color_config();
            show_tree_lines_ = IniConfig::instance().get_bool("display", "show_tree_lines", true);
            url_hyperlink_ = IniConfig::instance().get_bool("display", "url_hyperlink", true);  // Y24.27: cache once
            ...
            if (lyric_bar_height_ < 4) lyric_bar_height_ = 4;   // need ≥3 lyric lines + 2 borders
            if (lyric_bar_height_ > 12) lyric_bar_height_ = 12;
            ...
            // Load icon style config (emoji|ascii|auto) + emoji width probing
            {
                ...
                if (style == "emoji") {
                    ...
                    if (probed == 2) {
                        ...
                        IconManager::set_style(IconStyle::ASCII);
                        LOG(fmt::format("[UI] Auto icon style: ASCII (probed={}, terminal={})",
                            probed, TerminalDetector::terminal_name()));
                    }
                }
                ...
                LOG(fmt::format("[UI] Emoji width final: {} (override={}, probed={}, recommended={})",
                    get_emoji_width(),
                    g_emoji_width_override,
                    g_emoji_width_probed,
...
void UI::set_lyric_bar_requested(bool requested) {
            if (lyric_bar_requested_ == requested) return;
...
void UI::draw(AppMode mode, const std::vector<DisplayItem>& list, int selected, const MPVController::State& state, int view_start, AppState app_state, TreeNodePtr playback_node, int marked_count, const std::string& search_query, int current_match, int total_matches, TreeNodePtr selected_node, const std::vector<DownloadProgress>& downloads, bool visual_mode, int visual_start, const std::vector<PlaylistItem>& playlist, int playlist_index, //Play mode + INFO play-context (7-line: 3 history + current + 3 next)
                  PlayMode play_mode, const std::vector<std::string>& history_titles, const std::vector<int>& next_indices) {
            getmaxyx(stdscr, h, w);
            // NULL window guard: safe_wresize may null the window on failure; skip this frame to avoid dereferencing NULL
            if (!left_win || !right_win || !status_win || !lyric_win) return;
            ...
            if (new_top_h < 5) { new_top_h = 5; }  // match existing min guard

            //Detect whether window size changed (incl. L-mode active↔inactive transition → full redraw)
            bool size_changed = (new_left_w != left_w || new_right_w != right_w ||
                                 new_top_h != top_h || w != cols_ ||
                                 lyric_active != last_lyric_bar_active_);
            ...
            int title_max = left_w - 6; // left border 1 + space 1 + title + space 1 + top-right protection 3
            if (title_max < 5) title_max = 5;  // Minimum reservation
            ...
            const bool use_emoji_title = title_emoji_ &&
                (g_emoji_width_probed == 2 || g_emoji_width_override == 2);
            switch (mode) {
                case AppMode::RADIO: mode_str = use_emoji_title ? "📻 RADIO" : "[R] RADIO"; break;
                case AppMode::PODCAST: mode_str = use_emoji_title ? "🎤 PODCAST" : "[P] PODCAST"; break;
                case AppMode::FAVOURITE: mode_str = use_emoji_title ? "💖 FAVOURITE" : "[F] FAVOURITE"; break;
                case AppMode::HISTORY: mode_str = use_emoji_title ? "📜 HISTORY" : "[H] HISTORY"; break;  //new
                case AppMode::ONLINE: mode_str = use_emoji_title
                    ? fmt::format("🔍 ONLINE [{}]", ITunesSearch::get_region_name(OnlineState::instance().current_region))
                    : fmt::format("[O] ONLINE [{}]", ITunesSearch::get_region_name(OnlineState::instance().current_region));
                    break;
                case AppMode::ACCOUNT: mode_str = use_emoji_title ? "Ｙ ACCOUNT" : "[Y] ACCOUNT"; break;  // Y01 (fullwidth Y: glibc wcwidth=2)
                case AppMode::BILIBILI: mode_str = use_emoji_title ? "Ｂ BILIBILI" : "[B] BILIBILI"; break;  // Y15 (fullwidth B: glibc wcwidth=2)
                case AppMode::TIKTOK: {
                    const std::string& r = TikTokRegion::current();
                    // Y24.13: CN region = Douyin (douyin.com); show Douyin in the title.
                    if (r == "CN") mode_str = use_emoji_title ? fmt::format("🎵 Douyin [{}]", r) : fmt::format("[T] Douyin [{}]", r);
                    else mode_str = use_emoji_title ? fmt::format("🎵 TIKTOK [{}]", r) : fmt::format("[T] TIKTOK [{}]", r);
                    break;
                }
                case AppMode::IPTV: mode_str = use_emoji_title ? "📺 IPTV" : "[I] IPTV"; break;  // Y24.50
            }
            if (visual_mode) mode_str = use_emoji_title ? "§ VISUAL §" : "[V] VISUAL";
            if (marked_count > 0) mode_str += use_emoji_title
            ...
            if (tree_line_width < 10) tree_line_width = 10;  // Minimum width protection
            ...
            protect_border(left_win, left_w, top_h, -1);  // -1 = draw the whole top border as HLINE (no skip)
            mvwprintw(left_win, 0, 2, " %s ", title_display.c_str());
            mvwaddch(left_win, 0, left_w - 1, ACS_URCORNER);  // top-right corner protection (redrawn when title approaches)
            mvwaddch(left_win, 0, left_w - 2, ACS_HLINE);

            wnoutrefresh(left_win);  // double-buffer optimization

            // Right Panel
            werase(right_win);  // just clear; the border is drawn uniformly by protect_border at the end of draw_info
            ...
            draw_info(right_win, state, app_state, playback_node, marked_count,
                      ...
                      visual_mode, right_inner_width, playlist, playlist_index, play_mode,
                      history_titles, next_indices);
            ...
            if (!pending_osc8_.empty()) {
                ...
                int sy = 0, sx = 0; getyx(curscr, sy, sx);
                for (const auto& e : pending_osc8_) Utils::emit_osc8_link(e.row, e.col, e.visible, e.url, e.id);
...
void UI::draw_help(WINDOW* win, const MPVController::State& state, int cw) {
            (void)win; (void)state; (void)cw; // keep params for future use
            ...
            static const std::vector<std::string> help_lines = {
                ...
                "",
                "---- Playlist ----",
                "  (implicit) The peers (siblings) of the playing episode are the playlist; auto-advance per play mode",
                "  C          Clear playlist (keep playing) — stop auto-advance, current track keeps playing",
                "  L          LYRIC bar toggle; in F mode → transcribe selected/v-marked (whisper.cpp→.srt)",
                "  Ctrl+L     Cycle theme (22 palettes: Solarized Dark/Gruvbox/Nord/Dracula/Catppuccin/...)",
                ...
                "      video:  + zoom in | - zoom out | = reset zoom | f fullscreen | A aspect | d deinterlace",
                "      sub:    F/G size -/+ | z/Z sync -/+ | r/R pos up/down | v hide-show | j/J track next/prev",
                "      audio:  # cycle track | m mute",
                "      osd:    o progress | O level | i stats | I stats-toggle",
                "      other:  l ab-loop | s screenshot | S screenshot(no sub) | 1-8 contrast/bright/gamma/sat",
            ...
            if (help_y < 0) help_y = 0;  // when terminal is too short, stick to the top, to avoid newwin returning NULL on negative coords

            WINDOW* help_win = newwin(help_h, help_w, help_y, help_x);
            if (!help_win) return;  // NULL guard: avoid box(NULL) crash
            ...
            auto draw_content = [&]() {
                werase(help_win);
                box(help_win, 0, 0);
                if (scroll_offset < 0) scroll_offset = 0;  // defensive clamp
                int y = 1;
            ...
            while ((ch = wgetch(help_win)) != 'q' && ch != '?' && ch != 27) {
                if (ch == 'k' || ch == KEY_UP) {
                    ...
                    if (needs_scroll) {
                        scroll_offset = std::max(0, std::min(content_height - help_h + 2, scroll_offset + 5));
                        draw_content();
                    }
                } else if (ch == 'g') {
                    //Jump to top
