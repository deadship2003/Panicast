...
std::vector<TranscriptSegment> JsonSubtitleParser::parse(const std::string& content) const {
    ...
    try {
        ...
        if (j.is_object() && j.contains("segments") && j["segments"].is_array()) segs = &j["segments"];
        else if (j.is_array()) segs = &j;  // bare array of segments
        if (!segs) return out;
        for (const auto& s : *segs) {
            ...
            if (!seg.text.empty()) out.push_back(seg);
        }
        std::sort(out.begin(), out.end(),
                  [](const TranscriptSegment& a, const TranscriptSegment& b) { return a.start < b.start; });
...
std::vector<TranscriptSegment> SrtSubtitleParser::parse(const std::string& content) const {
    ...
    while (std::getline(ss, line)) {
        ...
        if (!parse_cue_times(line, start, end)) continue;
        std::string body, l;
        ...
        TranscriptSegment seg;
        seg.start = start; seg.end = end; seg.text = trim(body);
        if (!seg.text.empty()) out.push_back(seg);
    }
    return out;
}
...
static double parse_lrc_tag(const std::string& s) {
    ...
    for (char c : before) if (!std::isdigit((unsigned char)c)) return -1;  // metadata (ti/ar/...)
    try {
...
std::vector<TranscriptSegment> LrcSubtitleParser::parse(const std::string& content) const {
    ...
    while (std::getline(ss, line)) {
        ...
        while (pos < line.size() && line[pos] == '[') {
            ...
            if (t < 0) break;  // metadata tag → not a lyric timestamp
            stamps.push_back(t);
            pos = close + 1;
        }
        ...
        for (double t : stamps) {
            TranscriptSegment seg;
            seg.start = t; seg.end = t;  // LRC has no end; line stays until the next cue
            seg.text = text;
            out.push_back(seg);
        }
    }
...
std::vector<TranscriptSegment> TextWithTimestampsParser::parse(const std::string& content) const {
    ...
    auto flush = [&]() {
        if (in_seg) {
            ...
            if (!cur.text.empty()) { if (cur.end < cur.start) cur.end = cur.start; out.push_back(cur); }
...
std::string SubtitleParserRegistry::hint_for_url(const std::string& url_or_path) {
    ...
    if (ends_with(url_or_path, ".lrc")) return "lrc";
    if (ends_with(url_or_path, ".json")) return "json";
    if (ends_with(url_or_path, ".vtt")) return "vtt";
    if (ends_with(url_or_path, ".srt")) return "srt";
    return "";
}
...
std::vector<TranscriptSegment> SubtitleParserRegistry::parse(const std::string& content,
    ...
    if (t.empty()) {
        ...
        if (first != std::string::npos) {
            ...
            if (c == '{') t = "json";
            else if (content.compare(first, 6, "WEBVTT") == 0) t = "vtt";
            else if (c == '[') {
                ...
                while (j < content.size() &&
                       (content[j] == ' ' || content[j] == '\t' || content[j] == '\r' || content[j] == '\n'))
                    ++j;
                ...
                while (std::getline(scan, l) && scanned < 20) {
                    ...
                    if (lb != std::string::npos && lb + 1 < ll.size() && ll[lb] == '[') {
                        ...
                        if (cl != std::string::npos && parse_lrc_tag(ll.substr(lb + 1, cl - lb - 1)) >= 0) {
                            t = "lrc"; break;
                        }
                    }
                    if (is_hms_only(ll)) { t = "twt"; break; }  // Y24.9: omny TextWithTimestamps
                }
            }
        }
    }
    if (t == "json") return JsonSubtitleParser{}.parse(content);
    if (t == "vtt")  return VttSubtitleParser{}.parse(content);
    if (t == "lrc")  return LrcSubtitleParser{}.parse(content);
    if (t == "twt")  return TextWithTimestampsParser{}.parse(content);  // Y24.9
    return SrtSubtitleParser{}.parse(content);  // srt default
}

// ── Timed lookups ──
int find_at(const std::vector<TranscriptSegment>& segs, double time) {
    if (segs.empty() || time < 0) return -1;
    int lo = 0, hi = (int)segs.size() - 1, ans = -1;
    while (lo <= hi) {
        int mid = (lo + hi) / 2;
        if (segs[mid].start <= time) { ans = mid; lo = mid + 1; }
        else hi = mid - 1;
    }
...
std::vector<int> find_active(const std::vector<TranscriptSegment>& segs, double time) {
    std::vector<int> out;
    if (segs.empty() || time < 0) return out;
    int n = (int)segs.size();
    for (int i = 0; i < n; ++i) {
        if (segs[i].start > time) break;  // sorted → no later segment can be active
        double eff_end = (segs[i].end > segs[i].start)
                         ? segs[i].end
                         : (i + 1 < n ? segs[i + 1].start : 1e18);
        if (time < eff_end) out.push_back(i);
    }
    return out;
}
