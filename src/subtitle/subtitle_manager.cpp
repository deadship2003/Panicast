...
static std::string infer_type(const std::string& mime_or_type, const std::string& url) {
    ...
    if (t.find("vtt") != std::string::npos) return "vtt";
    if (t.find("srt") != std::string::npos || t.find("subrip") != std::string::npos) return "srt";
    if (t.find("lrc") != std::string::npos) return "lrc";
    // Y24.9: detect omny "TextWithTimestamps" by the URL query param (no MIME type / extension).
    std::string ul = url;
    for (auto& c : ul) c = (char)std::tolower((unsigned char)c);
    if (ul.find("format=textwithtimestamps") != std::string::npos) return "twt";
...
static int type_rank(const std::string& t) {
    ...
    if (t == "json") return 2;
    if (t == "lrc") return 1;
    if (t == "twt") return 1;  // parseable (omny fallback) but not mpv-native
    return 0;  // unknown
}
...
void SubtitleManager::detect_from_rss(TreeNodePtr node, const std::string& url, const std::string& type) {
    if (!node || url.empty()) return;
    ...
    if (node->has_subtitle && !node->subtitle_url.empty()) {
        ...
        if (new_rank <= cur_rank) {
            ...
            LOG(fmt::format("[Subtitle] RSS skip: {} (type={}, rank={}) — keep {} (type={}, rank={})",
                            url, new_type.empty() ? "auto" : new_type, new_rank,
                            node->subtitle_url, node->subtitle_type.empty() ? "auto" : node->subtitle_type, cur_rank));
            return;
        }
    }
    ...
    LOG(fmt::format("[Subtitle] RSS detected: {} (type={}, kept)", url, new_type.empty() ? "auto" : new_type));
}
...
std::string SubtitleManager::probe_sidecar(TreeNodePtr node) {
    ...
    if (!sc.empty() && node->subtitle_url.empty()) {
        ...
        LOG(fmt::format("[Subtitle] local sidecar detected for '{}': {} (source=asr)", node->title, sc));
    }
    return sc;
}
...
void SubtitleManager::load_async(TreeNodePtr node, ThreadPool& pool) {
    ...
    pool.submit([this, node]() {
        ...
        bool is_local = false;
        if (!sidecar.empty()) { src = sidecar; is_local = true; }
        ...
        if (src.rfind("http", 0) == 0) {
            ...
            if (content.empty()) fail_reason = "online fetch returned empty (network/HTTP/proxy?)";
        } else {
            std::ifstream f(src, std::ios::binary);
            if (!f) fail_reason = "sidecar file not readable";
            else content.assign((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>());
        }
        if (fail_reason.empty() && content.empty()) fail_reason = "empty content";
        ...
        if (fail_reason.empty()) {
            ...
            std::string hint = is_local ? SubtitleParserRegistry::hint_for_url(src)
                                        : (node->subtitle_type.empty()
                                               ? SubtitleParserRegistry::hint_for_url(src)
                                               : node->subtitle_type);
            segs = SubtitleParserRegistry::instance().parse(content, hint);
            LOG(fmt::format("[Subtitle] parsed {} segments ({} bytes, type={})",
                            segs.size(), content.size(), hint.empty() ? "auto" : hint));
            if (segs.empty()) fail_reason = "parsed 0 segments (format unrecognized or empty transcript)";
        }
...
void SubtitleManager::set_pending(const std::vector<TranscriptSegment>& segs, const std::string& url) {
    std::lock_guard<std::mutex> lk(mtx_);
    pending_ = segs;
    pending_url_ = url;
    pending_ready_ = true;
...
void SubtitleManager::download_sidecar(TreeNodePtr node, const std::string& local_file, ThreadPool& pool) {
    if (!node || !node->has_subtitle || node->subtitle_url.empty()) return;
    if (node->subtitle_url.rfind("http", 0) != 0) return;  // only fetch remote URLs
    std::string sub_url = node->subtitle_url;
    std::string type = node->subtitle_type;
    if (type.empty()) type = SubtitleParserRegistry::hint_for_url(sub_url);
    std::string sidecar = local_file + ext_for_type(type);
    pool.submit([sub_url, sidecar, type]() {
        std::string content = Network::fetch(sub_url);
        if (content.empty()) { LOG(fmt::format("[Subtitle] sidecar download empty: {}", sub_url)); return; }
