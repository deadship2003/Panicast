...
- **D14 — Media 域从 TreeNode 逐步收敛（M2 主线）**。问题定位：now-playing 身份当前分裂为三种表示——内存 `TreeNodePtr`、播放路径串（`MPVController::State::current_url`，Y23.9 起对缓存项是**原始本地路径**非 `file://`）、源 URL 串——三者从不统一；本地/流式判定 `url[0]=='/'||file://` 在 app_input/app_remote 复制 4 份。收敛 = 以**真实绝对源 URL**为唯一身份，统一 tree/DB/H/F/远程。每步绿、可回退。
  ...
  - [~] **测试镜像漂移修复**（test_units 抄 classify/parse_time_string/escape_sql 三副本→测副本非真实，假阳性）。**classify ✅ 2026-08-13**：删 panicast_test 副本（URLType/PATTERNS/classify），链入真实 URLClassifier（CMake `target_sources`，纯函数零依赖）；测试改 `URLClassifier::classify`+`using namespace panicast`。ctest 44/44 全过（现有 case 与真实巧合一致，镜像风险根除）。**parse_time/escape_sql 待评估**（见 CHANGELOG）：parse_time 链入拖 event_log/sleep_timer（test 变重，漂移=InvalidInput 0 vs -1）；escape_sql 生产 0 调用（死代码）+ 漂移（strip NUL），倾向删除而非拆分。
