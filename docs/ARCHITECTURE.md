...
| `app` | 应用层：`app_run`（主循环：D35 抽 startup/shutdown bookend、D37 抽 tree-locked 显示构建 phase，run() loop 收敛为信号/定时+状态计算+下载/视图+draw+输入）、`app_persistence`（D23）、`app_feeds`（D32）、`app_flatten`（D33）、`app_input`（键派发）、`app_playback/download/search/subscriptions/navigation/...`、modes/ |
...
- **上帝对象/文件**：`App`（`app.h` 668 行声明）、`app_run.cpp`(557，机械缝挖尽；run() god-方法 D35 抽 startup/shutdown bookend、D37 抽 tree-locked 显示构建 phase，剩 loop 各 phase 与外层局部缠绕待设计)、`mpv_controller.cpp`(904，D18-D20+D34 机械抽 wrapper/metadata/iptv/单条播放；D36 设计层抽 event_loop codec-info 块；剩 event_loop END_FILE 分支待设计)、`app_input.cpp`(903，硬编码 `switch(ch)`)。~~`ini_config.h`~~（原 1087 行 god-header，D24-D31 已迁 72 个方法出体，现 304 行纯声明头，**基本驯服**）。
