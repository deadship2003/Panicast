...
| `app` | 应用层：`app_run`（主循环：D35 后 run()=纯帧循环，startup/shutdown bookend 已 Extract Method 抽出；loop 体 286 行仍大但连贯）、`app_persistence`（D23 启动/退出 DB 持久化：load_data/load_persistent_data/save_persistent_data/restore_player_state）、`app_feeds`（D32 feed-loading 簇）、`app_flatten`（D33 tree-flatten 簇）、`app_input`（键派发）、`app_playback/download/search/subscriptions/navigation/...`、modes/ |
...
- **上帝对象/文件**：`App`（`app.h` 666 行声明）、`app_run.cpp`(550，机械缝挖尽；run() god-方法 D35 抽出 startup/shutdown bookend，剩 loop 体 286 行待进一步设计分解)、`mpv_controller.cpp`(904，D18-D20+D34 机械抽 wrapper/metadata/iptv/单条播放；D36 设计层抽 event_loop codec-info 块；剩 event_loop END_FILE 分支/run loop 待设计)、`app_input.cpp`(903，硬编码 `switch(ch)`)。~~`ini_config.h`~~（原 1087 行 god-header，D24-D31 已迁 72 个方法出体，现 304 行纯声明头，**基本驯服**）。
