...
public:
    ...
    // N04: PIN auth. The dynamic PIN is a random 4-digit code shown in a PodRadio popup; the
    ...
    std::string dynamic_pin() const { return dynamic_pin_; }
    std::string universal_pin() const { return universal_pin_; }
    ...
    RemoteCommandBus& command_bus() { return bus_; }
    RemoteControlInterface* control_interface() { return control_; }
    void register_session(RemoteSession* s);
    void unregister_session(RemoteSession* s);

private:
    void accept_loop();
    void ws_accept_loop();  // N06: accept WebSocket/HTTP connections (serves the BS client + WS bridge)
