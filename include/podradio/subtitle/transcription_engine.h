...
#include "podradio/core/types.h"  // TreeNodePtr
#include "podradio/subtitle/subtitle_parser.h"  // TranscriptSegment
...
public:
    void init(SubtitleManager* sm, ThreadPool* pool, MPVController* mpv = nullptr) { sm_ = sm; pool_ = pool; mpv_ = mpv; }
    ...
    void start_realtime(TreeNodePtr node, const std::string& url, bool is_streaming, bool is_video = false);
    void stop_realtime();          // kill + invalidate the running realtime job
    bool realtime_running() const { return realtime_gen_.load() > 0 && realtime_active_.load(); }

    bool busy() const { return active_.load() > 0 || !queue_empty(); }
...
private:
    ...
    std::atomic<bool> stop_offline_{false};  // Y24.21: stop the offline batch (kill whisper-cli + clear queue)
    ...
    void realtime_worker(TreeNodePtr node, std::string url, bool is_streaming, bool is_video, unsigned gen);
