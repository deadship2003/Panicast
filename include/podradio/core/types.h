...
enum class MediaType {
    Radio, Youtube, Bilibili, Tiktok, Iptv,
    OnlineAudio, OnlineVideo, LocalAudio, LocalVideo
};
...
struct TreeNode
{
    ...
    TreeNodeWeakPtr linked_node;     // weak_ptr to avoid circular references (runtime reference, may expire)
    std::string link_target_url;     // URL identifier of the link target (used for persistence and rebuild)
    ...
    bool is_yt_search_result = false;   // true = a YouTube search-result node (channel/video/playlist)
    bool is_yt_playlist = false;        // Y23.1: a YouTube playlist search-result (distinct icon from channel)
    bool is_yt_music = false;           // Y23.1: a YouTube music-category video search-result (🎵 icon)
    bool is_search_parent = false;      // Y23.1: a "Search History" container node (r/d no-op, l/enter expands)
    bool is_bili_up = false;            // Y23.2: a Bilibili UP-master node (followings/subscribed/search) → 👤 icon
    // ── Y22: Bilibili B-mode account children (mirror Y-mode History / Subscriptions) ─────────
    bool is_bili_followings = false;    // true = "Subscriptions" child (Bilibili followings) under a B account
    bool is_bili_history = false;       // true = "History" child (Bilibili watch history) under a B account
...
enum class URLType {
    ...
    BILIBILI_CHANNEL, BILIBILI_VIDEO, DOUYIN_USER, DOUYIN_VIDEO,  // Y16: B/T mode URLs
    TIKTOK_USER, TIKTOK_VIDEO  // Y24.11: T mode (tiktok.com/@user, /video/<id>, vm.tiktok.com shortlink)
};
