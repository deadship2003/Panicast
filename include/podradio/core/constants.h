...
//   tracks PODRADIO_FIX_SUFFIX without a manual fallback sync. Falls back below for direct g++.
...
#ifdef PODRADIO_VERSION_STRING
constexpr const char* VERSION     = PODRADIO_VERSION_STRING;
#else
constexpr const char* VERSION     = "PaniCast-V0.01";
#endif
...
constexpr int DOWNLOAD_COMPLETE_DISPLAY_SEC = 5;    // Display duration after a successful download (seconds)
constexpr int DOWNLOAD_FAILED_DISPLAY_SEC  = 8;     // How long a FAILED entry stays visible (seconds; 5–10 range)
constexpr int MAX_CONCURRENT_DOWNLOADS = 10;        // Max simultaneously active/visible download slots; excess are queued
