...
public:
    ...
    struct ProcResult { bool launched = false; int exit_code = -1; std::string stdout_out; std::string stderr_out; };
    ...
    // N04-fix: fork+exec a child with PR_SET_PDEATHSIG(SIGKILL) so it auto-dies when podradio dies
    ...
    //   down its whole subtree (e.g. yt-dlp's ffmpeg) on graceful shutdown (pkill podradio).
    ...
    static pid_t spawn_child(const std::string& exe, char* const argv[],
    ...
    static int utf8_char_display_width(unsigned char first_byte, const unsigned char* next_bytes, size_t avail);
