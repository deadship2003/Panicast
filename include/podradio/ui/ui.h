...
#include "podradio/ui/theme_manager.h"  // Y24.30: extracted from UI   // Y11: 12-palette table (struct Theme, themes(), THEME_COUNT)
...
#include "podradio/net/tiktok_region.h"  // Y24.12: T-mode region badge in the mode title
...
#include "podradio/ui/layout_guard.h"
#include "podradio/ui/border.h"
#include "podradio/theme/colors.h"
#include "podradio/theme/pairs.h"
...
extern volatile sig_atomic_t g_crash_sig;
    ...
    public:
        ...
        bool is_show_tree_lines() const { return show_tree_lines_; }
        ...
        bool is_lyric_bar_requested() const { return lyric_bar_requested_; }
        bool is_lyric_bar_active() const { return lyric_bar_active_; }
        void set_lyric_bar_active(bool active) { lyric_bar_active_ = active; }
        ...
        LyricManual lyric_manual() const { return lyric_manual_; }
        void set_lyric_manual(LyricManual m) { lyric_manual_ = m; }
        ...
        bool embedded_sub_confirmed() const { return embedded_sub_confirmed_; }
        ...
        static constexpr const char* INPUT_CANCELLED = "\x01" "CANCELLED" "\x01";  // special marker indicating user cancellation
        ...
        //   podradio/theme/themes.h) so the 15 palettes can be edited without touching UI code.
        //   struct Theme / THEME_COUNT / themes() are at namespace podradio scope (name lookup).
        ...
        //Cycle theme (Ctrl+L)
        void toggle_theme() { ThemeManager::instance().toggle(); EVENT_LOG(fmt::format("Theme: {}", ThemeManager::instance().current_name())); }

        //Current theme name
        std::string current_theme_name() const { return ThemeManager::instance().current_name(); }

        // Left panel geometry (for mouse-click hit testing): origin (0,0), height top_h, width left_w
        int get_left_w() const { return left_w; }
        int get_top_h() const { return top_h; }
        ...
        void apply_theme() { ThemeManager::instance().apply(); }  // Y24.30: delegate

    private:
        WINDOW *left_win = nullptr, *right_win = nullptr, *status_win = nullptr, *lyric_win = nullptr;  // initialized to nullptr
        ...
        bool title_emoji_ = true;  // border title Emoji switch (default Emoji; narrow-emoji terminals auto-fallback to ASCII)
        ...
        struct Osc8Emit { int row; int col; std::string visible; std::string url; std::string id; };
        ...
        // │   ░▒▓█ ★ PODRADIO V0.05B9n3e5g2RF ★ █▓▒░   [ URL ]   ░▒▓█ By author time █▓▒░ │
