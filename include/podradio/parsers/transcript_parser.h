...
#include "podradio/subtitle/subtitle_parser.h"  // TranscriptSegment + registry + find_at/find_active
...
public:
    ...
    static int find_at(const std::vector<TranscriptSegment>& segs, double time) {
        return podradio::find_at(segs, time);
    }
    static std::vector<int> find_active(const std::vector<TranscriptSegment>& segs, double time) {
        return podradio::find_active(segs, time);
    }

    static double parse_timestamp(const std::string& s) { return podradio::parse_timestamp(s); }
};
