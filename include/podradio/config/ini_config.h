// INI config management: read/write ~/.config/podradio/config.ini, includes status bar color config types.
#pragma once
...
namespace fs = std::filesystem;
    ...
    public:
        ...
        void save() {
            ...
            for (const auto& raw : raw_lines_) {
                ...
                if (a == std::string::npos) { f << raw << "\n"; continue; }
                if (line[a] == '#' || line[a] == ';') { f << raw << "\n"; continue; }
                if (line[a] == '[') {
                    size_t rb = line.find(']', a);
                    if (rb != std::string::npos) current_section = line.substr(a + 1, rb - a - 1);
                    f << raw << "\n";
                    continue;
                }
                size_t eq = line.find('=', a);
                if (eq == std::string::npos) { f << raw << "\n"; continue; }
        ...
        StatusBarColorConfig get_statusbar_color_config() {
            ...
            if (!colors_str.empty()) {
                ...
                while (std::getline(ss, token, ',')) {
                    try { cfg.custom_colors.push_back(std::stoi(token)); }
                    catch (...) {}
                }
            }
        ...
        bool get_tls_verify() const { return get_bool("network", "tls_verify", true); }
        bool get_reject_unsafe_url() const { return get_bool("network", "reject_unsafe_url", true); }
        int  get_network_timeout() const { return get_int("network", "timeout", 30); }
        ...
        bool get_url_hyperlink() const { return get_bool("display", "url_hyperlink", true); }
        ...
        PlayMode get_play_mode() const {
            ...
            if (v == "repeat" || v == "r") return PlayMode::REPEAT;
            if (v == "shuffle" || v == "s" || v == "random") return PlayMode::SHUFFLE;
        ...
        float get_log_height_ratio() const { return get_float("display", "log_height_ratio", 0.3f); }
        ...
        int get_log_compress_height() const { return get_int("display", "log_compress_height", 23); }
        ...
        int get_display_state_refresh_ms() const { return get_int("display", "state_refresh_ms", 100); }
        ...
        bool get_display_lyric() const { return get_bool("display", "lyric", true); }
        int  get_display_lyric_lines() const { return get_int("display", "lyric_lines", 3); }
        ...
        bool get_display_lyric_bar() const { return get_bool("display", "lyric_bar", false); }
        int  get_display_lyric_bar_height() const { return get_int("display", "lyric_bar_height", 5); }

        // ── MPV playback config ([mpv] section) ───────────────────────────────────
        std::string get_mpv_vo() const { return get("mpv", "vo", "auto"); }
        std::string get_mpv_vid() const { return get("mpv", "vid", "auto"); }
        ...
        std::string get_mpv_ytdl_format() const { return get("mpv", "ytdl_format", "bestvideo+bestaudio"); }
        std::string get_mpv_user_agent() const { return get("mpv", "user_agent", "Mozilla/5.0 (X11; Linux x86_64; rv:128.0) Gecko/20100101 Firefox/128.0"); }
        std::string get_mpv_cache() const { return get("mpv", "cache", "yes"); }
        std::string get_mpv_demuxer_max_bytes() const { return get("mpv", "demuxer_max_bytes", "50MiB"); }
        std::string get_mpv_demuxer_max_back_bytes() const { return get("mpv", "demuxer_max_back_bytes", "25MiB"); }
        int get_mpv_cache_secs() const { return get_int("mpv", "cache_secs", 10); }
        ...
        int get_mpv_audio_cache_secs() const { return get_int("mpv", "audio_cache_secs", 5); }
        std::string get_mpv_audio_demuxer_max_bytes() const { return get("mpv", "audio_demuxer_max_bytes", "5MiB"); }
        std::string get_mpv_audio_demuxer_max_back_bytes() const { return get("mpv", "audio_demuxer_max_back_bytes", "2MiB"); }
        int get_mpv_audio_cache_pause_wait() const { return get_int("mpv", "audio_cache_pause_wait", 1); }
        bool get_mpv_tls_verify() const { return get_bool("mpv", "tls_verify", true); }
        bool get_mpv_keep_open() const { return get_bool("mpv", "keep_open", true); }
        ...
        std::string get_mpv_sub_align_x() const { return get("mpv", "sub_align_x", "center"); }
        std::string get_mpv_sub_align_y() const { return get("mpv", "sub_align_y", "bottom"); }
        std::string get_mpv_sub_visibility() const { return get("mpv", "sub_visibility", "yes"); }
        std::string get_mpv_sub_ass_override() const { return get("mpv", "sub_ass_override", "auto"); }
        // Y24.43: preferred subtitle language for selecting among multiple embedded tracks (mpv slang).
        std::string get_mpv_sub_lang() const { return get("mpv", "sub_lang", "en"); }
        ...
        int get_iptv_cache_hours() const { return get_int("iptv", "cache_hours", 24); }
        // Custom user m3u URLs (comma-separated) shown under I-mode → Custom.
        std::string get_iptv_custom_urls() const { return get("iptv", "custom_urls", ""); }
        // Y24.55: seconds a connected IPTV stream may stay data-less (core-idle, no codec, no
        //   download) before PodRadio reports it as likely off-air. Tunable so slow links can
        //   raise it. See MPVController IPTV detection in mpv_controller.cpp.
        int get_iptv_offair_detect_secs() const { return get_int("iptv", "offair_detect_secs", 12); }
        ...
        bool get_remote_enabled() const { return get_bool("remote", "enable", false); }
        int get_remote_port() const { return get_int("remote", "port", 8421); }
        std::string get_remote_bind() const { return get("remote", "bind", "127.0.0.1"); }
        std::string get_remote_auth_token() const { return get("remote", "auth_token", ""); }
        ...
        std::string get_remote_universal_pin() const { return get("remote", "universal_pin", "6696"); }
        // N05: UDP discovery port the APK broadcasts to in order to auto-find PodRadio instances.
        int get_remote_discovery_port() const { return get_int("remote", "discovery_port", 18430); }
        ...
        static std::string normalize_proxy(const std::string& input, bool* is_valid = nullptr) {
            ...
            if (s.empty()) return "";  // proxy disabled
            // Lowercase the scheme (before "://")
            size_t pos = s.find("://");
            if (pos == std::string::npos) { if (is_valid) *is_valid = false; return s; }
        ...
        bool is_url_safe(const std::string& url) const {
            if (!get_reject_unsafe_url()) return true;  // user explicitly disabled
    ...
    private:
        ...
        std::string resolve_cookies_path(const std::string& section, const std::string& key,
            ...
            if (!v.empty() && v[0] == '~') { const char* h = std::getenv("HOME"); if (h) v = std::string(h) + v.substr(1); }
            if (!v.empty() && v[0] == '/') return v;
            std::string base = Paths::get_data_dir();
            return base.empty() ? v : (base + "/" + v);
        }
        std::map<std::string, std::map<std::string, std::string>> data_;
        std::vector<std::string> raw_lines_;  // original file lines (incl. comments/blank lines/order), preserved for save()
        mutable std::shared_mutex cfg_mtx_;   // P2 (Y23.7): protect data_ (mpv thread reads, UI thread writes)
...
# podradio-YYYYMMDD.log). play_audio/play_video still tune cache-secs per audio/video.
cache = yes
...
# 如需自定义，请编辑 src/podradio.cpp 中 YtdlpRunner::run 调用 / To customize, edit the YtdlpRunner::run call in src
...
# 调试请通过环境变量或直接查看日志文件 ~/.local/share/podradio/podradio-YYYYMMDD.log (daily, kept 365 days) / For debugging, use env vars or view the log at ~/.local/share/podradio/podradio-YYYYMMDD.log (daily, kept 365 days)
