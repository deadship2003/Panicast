...
public:
    ...
    bool initialize();
    static void set_cli_overrides(const std::string& vo, const std::string& vid, const std::string& ao);  // CLI --vo/--vid/--ao overrides
    ...
    void play_video(const std::string& url, const std::string& audio_file = "", const std::string& sub_file = "");
    void play(const std::string& url, bool force_video = false, const std::string& audio_file = "", const std::string& sub_file = "");
    ...
    void set_keep_open(bool keep);  // keep-open=yes pauses at EOF; no lets END_FILE fire for auto-advance
    ...
    struct State {
        ...
        std::string hwdec_current;    // F27: active hwdec method (e.g. "vaapi-copy"); empty/"no" = software decode
    ...
    //   explanation); both reach the on-screen LOG area and podradio.log via EVENT_LOG.
...
private:
    ...
    bool vo_gpu_ = false;
    bool restart_info_logged_ = false;  // F28: one-shot per-track log guard, fired on PLAYBACK_RESTART (codec + hwdec)
    std::atomic<int64_t> last_loadfile_ms_{0};  // Y24.27: atomic (was steady_clock::time_point — data race between UI + mpv threads)
    std::atomic<bool> logging_load_{false};  // Y24.17: load window — log mpv INFO messages (AO/demuxer/cache) during BUFFERING
    ...
    std::chrono::steady_clock::time_point file_loaded_time_{};  // when FILE_LOADED fired (off-air/audio-only timing)
    ...
    bool had_playback_started_ = false;           // Y24.55: true once PLAYBACK_RESTART fired → distinguishes #12 (mid-playback drop) from #1-10 (load/init failure)
